...
本仓库目前包含医院物价折价改造项目的设计文档、.NET 8 规则中心源码、HIS 客户端示例、Oracle SQL 初始化脚本和自动化测试。
...
- `src/`：规则中心源码，包含 `Pricing.RuleCenter.Api`、`Pricing.RuleCenter.Application`、`Pricing.RuleCenter.Core`、`Pricing.RuleCenter.Infrastructure` 项目。
- `tests/`：xUnit 自动化测试。
...
Git 提交信息必须使用中文，使用简洁的祈使句，例如 `规范规则中心项目目录结构`。后续提交应聚焦单一逻辑变更，避免把无关格式调整混入同一提交。

合并请求说明应使用中文，包含变更摘要、影响的文档或模块、关键业务规则变化以及未确认问题。涉及 HIS 界面或流程时，补充截图或流程说明。相关任务可引用 `07-开发任务清单-物价折价改造.md`。
