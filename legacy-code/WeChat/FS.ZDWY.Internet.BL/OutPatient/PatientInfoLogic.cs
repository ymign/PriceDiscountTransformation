...
{
    public class PatientInfoLogic : SqlSugar.DbContext<FS.ZDWY.Internet.Models.COM_PATIENTINFO>
    {
        ...
        //      return dt;
        //  }
        public DataTable QueryPatientList(string card_no, string medicalNo, string certifcateType, string certifcateNo)
        {
            //            string sql = @"SELECT t.* from (select 
            //                        p.name ""name""  --姓名  必填  
            //                        ,(case p.sex_code when 'M' then '1'
            //                            when 'F' then '2'
            //                            else '9' end)  ""sex"" --性别  必填  
            //                        ,fun_get_age(p.birthday) ""age"" --年龄  必填  
            //                        ,p.birthday ""birth"" --出生日期    
            //                        ,p.home_now ""address"" --地址    
            //                        ,p.home_tel ""mobile""  --电话    
            //                        ,p.idcardtype ""certifcateType""  --证件类型  必填  int
            //                        ,p.idenno ""certifcateNo""  --证件号码  必填  
            //                        ,'1' ""cardType""  --诊疗卡类型 必填  int
            //                        ,p.card_no ""cardNo""  --诊疗卡号码 必填  
            //                        ,p.card_no ""patientId"" --患者ID  必填  
            //                        ,p.card_no ""medicalNo"" --病历号
            //                        ,(SELECT Max(i.patient_no) FROM fin_ipr_inmaininfo i WHERE i.card_no=p.card_no ) ""admissionNo""
            //                        ,p.card_no ""visitNo""
            //                        ,CASE WHEN p.idcardtype = '11' AND TRUNC(MONTHS_BETWEEN(SYSDATE, p.birthday) / 12) >= 65 THEN '1' ELSE to_char(NVL(p.elderlyvoucherflag,'0')) END ""elderlyVoucherFlag""
            //                        from com_patientinfo p 
            //                        where  p.card_no not like '9%'
            //and p.card_no not like '10%'
            //--and not regexp_like(p.card_no, '[[:alpha:]]')
            //--and p.card_no not like '%L%'
            //and p.card_no not like '%C%'
            //and (p.card_no=:card_no or 'ALL'=:card_no)
            //and (p.Case_No=:medicalNo or 'ALL'=:medicalNo) 
            //--and (p.idcardtype=:certifcateType and p.idenno=:certifcateNo)
            //and (p.idenno=:certifcateNo or 'ALL'=:certifcateNo)
            //order BY  nvl((select max(r.reg_date) from  fin_opr_register r where p.card_no=r.card_no ),to_date('0001-01-01','yyyy-MM-dd')) DESC,p.oper_date DESC,p.lreg_date DESC nulls last) t WHERE ROWNUM<2";
            //            var queryData = Db.Ado.GetDataTable(sql, new List<SugarParameter>(){
            //                            new SugarParameter(":card_no", string.IsNullOrEmpty(card_no)?"ALL":card_no),
            //                            new SugarParameter(":medicalNo", string.IsNullOrEmpty(medicalNo)?"ALL":medicalNo),
            //                            new SugarParameter(":certifcateType",certifcateType),
...
and p.card_no not like '10%'
and p.card_no not like '%C%'
and (p.card_no='{0}' or 'ALL'='{0}')
and (p.Case_No='{1}' or 'ALL'='{1}') 
--and (p.idcardtype='{2}' and p.idenno='{2}')
and (p.idenno='{3}' or 'ALL'='{3}')
order BY  nvl((select max(r.reg_date) from  fin_opr_register r where p.card_no=r.card_no ),to_date('0001-01-01','yyyy-MM-dd')) DESC,p.oper_date DESC,p.lreg_date DESC nulls last) t WHERE ROWNUM<2";

            string strSql = string.Format(sql, string.IsNullOrEmpty(card_no) ? "ALL" : card_no, string.IsNullOrEmpty(medicalNo) ? "ALL" : medicalNo, certifcateType, string.IsNullOrEmpty(certifcateNo) ? "ALL" : certifcateNo);
        ...
        {
            string tname = string.IsNullOrEmpty(name) ? "ALL" : name;
            string tvisitNo = string.IsNullOrEmpty(visitNo) ? "ALL" : visitNo;
            string sql = @"SELECT t.* from (select 
p.card_no ""patientId"",-- 患者ID
p.card_no ""visitNo"" ,-- 门诊号
p.idcardtype ""certifcateType"",--    证件类型
p.idenno ""certifcateNo"",-- 证件号码
p.name  ""name"",-- 姓名
p.sex_code ""sex""  ,-- 性别
decode(p.birthday,null,'',fun_get_age(p.birthday)) ""age"" ,-- 年龄
p.birthday ""birth"",--    出生日期
p.home_now ""address""  ,--   地址
p.home_tel ""mobile"",--   电话
NVL(p.elderlyvoucherflag,'0') ""elderlyVoucherFlag""
from com_patientinfo p 
where  p.card_no not like '9%'
and p.card_no not like '10%'
and p.card_no not like '%C%'
and (p.idenno='{0}' and (p.idcardtype = '{1}' or p.idcardtype is null))
and (p.name='{2}' or 'ALL'='{2}')
and (p.card_no='{3}' or 'ALL'='{3}')
order BY  nvl((select max(r.reg_date) from  fin_opr_register r where p.card_no=r.card_no ),to_date('0001-01-01','yyyy-MM-dd')) DESC,p.oper_date DESC,p.lreg_date DESC nulls last) t WHERE ROWNUM<2";
            string strSql = string.Format(sql, certifcateNo, certifcateType, tname, tvisitNo);
            var queryData = Db.Ado.GetDataTable(strSql);
            return queryData;
        }
