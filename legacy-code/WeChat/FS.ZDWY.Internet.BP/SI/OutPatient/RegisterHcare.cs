...
namespace FS.ZDWY.Internet.BP.SI.OutPatient
{
    ...
    /// 挂号减免
    /// </summary>
    public class RegisterHcare : AbstractService<FS.ZDWY.Internet.Models.Views.OutPatient.ComPatient, GDSI.Models.OutParam.OutParamBizh110104>
    {
        ...
        protected override int ConvertModelToSendMessage(FS.ZDWY.Internet.Models.Views.OutPatient.ComPatient r, out string xml, params object[] appendParams)
        {
            ...
            try
            {
                ...
                #region 患者信息
                AppendChildNode(doc, input, "bka017", r.OperTime.ToString("yyyyMMdd"));
                ...
                if (undrg != null && !string.IsNullOrEmpty(undrg.ID))
                {
                    ...
                    AppendChildNode(doc, row, "bka051", r.OperTime.ToString("yyyyMMdd"));
                    AppendChildNode(doc, row, "bka055", "次");
                    AppendChildNode(doc, row, "bka056", (r.RegFee+r.OwnDigFee).ToString());
                    AppendChildNode(doc, row, "bka057", "1");
                    AppendChildNode(doc, row, "bka058", (r.RegFee + r.OwnDigFee).ToString());
                ...
                {
                    this.ErrorMsg = "调用门诊挂号接口(bizh110104)异常。获取挂号费用编码失败!";
                    return -1;
                }
                #endregion
