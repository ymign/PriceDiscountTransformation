...
,
        ...
        /// 发票号锁定，防止并发重号
        /// </summary>
        public static string LockInvoiceNo = @"UPDATE fin_com_invoice t
...
sysdate)";
  ...
  and tt.pact_code='1'
  and t.id in(select min(t.id) from FIN_OPR_SCHEMA t where t.dept_code not in('6068','7021') 
  and t.stop<>'1'
 ...
 -- and t.end_time>=sysdate
  ...
  --and fun_get_noon(sysdate)=t.noon_code
   group by t.doct_code,t.noon_code)
...
update fin_opr_schema s --医师出诊表
 ...
 where s.id = '{0}'
   ...
   and s.stop<>'1'";
        ...
        public static string updatebooking = @"update fin_opr_booking p
                                                set p.valid_flag='{1}',
                                                    p.see_flag='{2}',
                                                    p.reg_id='{3}'
