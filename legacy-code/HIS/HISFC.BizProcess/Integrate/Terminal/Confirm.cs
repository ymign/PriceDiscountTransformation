...
namespace Neusoft.HISFC.BizProcess.Integrate.Terminal
{
    ...
    public class Confirm : IntegrateBase
    {
        ...
        /// </summary>
        protected Neusoft.FrameWork.Management.ControlParam serviceControler = new Neusoft.FrameWork.Management.ControlParam();
        ...
        protected void GetEnvironment()
        {
            ...
            // 使用更安全的方式判断是否开发环境
            if (AppDomain.CurrentDomain.FriendlyName.ToUpper().Contains("VSHOST") ||
                AppDomain.CurrentDomain.FriendlyName.ToUpper().Contains("DEVENV"))
            {
                return;
            }
        ...
        /// <summary>
        /// 获取当前操作环境
        /// yhm 2025-05-14 注释 System.Diagnostics.Process.GetCurrentProcess().ProcessName.ToUpper() == "DEVENV" 有性能问题 它在资源紧张（尤其是内核资源或性能计数器损坏）时，会触发异常：
        ///System.InvalidOperationException: Couldn't get process information from performance counter.
        /// </summary>
        protected void GetEnvironmentOld()
        {
            CurrentOperEnvironment.ID = serviceTerminalConfirm.Operator.ID;
            CurrentOperEnvironment.Name = serviceTerminalConfirm.Operator.Name;
        ...
        public Result QueryByInpaRetient(string inpatientNO, string operDept, ref ArrayList al)
        {
            this.SetDB(serviceTerminalConfirm);
            // 获取
            this.intReturn = serviceTerminalConfirm.QueryByInpaRetientmList(inpatientNO, operDept, ref al);
            if (this.intReturn == -1)
            {
                return Result.Failure;
            }
        ...
        public Result QueryDiagnosis(string name, string code, ref ArrayList al)
        {
            this.SetDB(serviceTerminalConfirm);
            // 获取
            this.intReturn = serviceTerminalConfirm.QueryDiagnosisList(name, code, ref al);
            if (this.intReturn == -1)
            {
                return Result.Failure;
            }
        ...
        public Result QueryConfirmInfoByClinicNoCancel(string clinicNo, string deptID, ref List<Neusoft.HISFC.Models.Terminal.TerminalApply> apply)
        {
            this.SetDB(serviceTerminalConfirm);
            // 获取
       ...
       /// <summary>
        ...
        public int CancelInaptientTerminal(string moOrder, string execOrder)
        {
            ...
            List<Neusoft.HISFC.Models.Terminal.TerminalConfirmDetail> list = this.serviceTerminalConfirm.QueryTerminalConfirmDetailByMoOrderAndExeMoOrder(moOrder, execOrder);
            if (list != null)
            {
                foreach (Neusoft.HISFC.Models.Terminal.TerminalConfirmDetail terminalApply in list)
                {
        ...
        public int CancelInaptientTerminal2(Neusoft.HISFC.Models.Fee.ReturnApply quitFee)
        {
            this.serviceTerminalConfirm.SetTrans(this.trans);

            //根据处方号和项目流水号一对一的取出当前退费项目的确认记录
            List<Neusoft.HISFC.Models.Terminal.TerminalConfirmDetail> list = this.serviceTerminalConfirm.QueryTerminalConfirmDetailByQuitFee(quitFee);
        ...
        public int UpdateTerminalDetailRecipe(ArrayList alQuitFeeItemList)
        {
            ...
            {
                foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList feeItemList in alQuitFeeItemList)
                {
