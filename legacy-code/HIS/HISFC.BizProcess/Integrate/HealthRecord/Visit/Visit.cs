...
namespace Neusoft.HISFC.BizProcess.Integrate.HealthRecord.Visit
{
    ...
    public class Visit : IntegrateBase
    {
        ...
        /// 短信收发管理类
        /// </summary>
        protected static Neusoft.HISFC.BizLogic.HealthRecord.Visit.SMS SMSManager = new Neusoft.HISFC.BizLogic.HealthRecord.Visit.SMS();
        ...
        /// <param name="cardNo">病历号</param>
        /// <returns>影响的行数；-1－失败</returns>
        public int UpdateLinkWayPhone(Neusoft.HISFC.Models.HealthRecord.Visit.LinkWay linkway)
        {
            this.SetDB(linkWayManager);

            int intReturn = linkWayManager.UpdateLinkWayPhone(linkway);
            if (intReturn != 1)
            {
                this.Err = linkWayManager.Err;
                return -1;
        ...
        {
            this.SetDB(visitRecordManager);
            int a = SMSManager.InsertSendSMS(sms);
            if (a == -1)
            {
                return -1;
            }
        ...
        public int InsertReceiveSMS(Neusoft.HISFC.Models.HealthRecord.Visit.ReceiveSMS sms)
        {
            ...
            if (SMSManager.InsertReceiveSMS(sms) == -1)
            {
                return -1;
            }
            return 0;
        ...
        {
            this.SetDB(visitRecordManager);
            if (SMSManager.UpdateSmID() <= 0)
            {
                return -1;
            }
            return SMSManager.GetSmid();
        }
        ...
        {
            this.SetDB(visitRecordManager);
            if (SMSManager.UpdateSrcid() <= 0)
            {
                return -1;
            }
            return SMSManager.GetSrcid();
        }
        ...
        {
            this.SetDB(visitRecordManager);
            return SMSManager.QuerySendRecordByPationNo(patientno);
        }
        ...
        {
            this.SetDB(visitRecordManager);
            return SMSManager.QuerySendRecordBySmid(smid);
        }
        ...
        {
            this.SetDB(visitRecordManager);
            return SMSManager.QueryReceiveRecode(smid);
        }
        #endregion
    }
}
