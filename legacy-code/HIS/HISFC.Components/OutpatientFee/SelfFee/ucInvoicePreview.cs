            ...
            for (int i = 1; i < 50; i++)
            {
                ...
                else
                {
                    ...
                    if (detail.BalanceBase.FT.TotCost > 0)
                    {
                        ...
                        if (invoicePreviewType == "1" && this.regInfo.Pact.PayKind.ID == "03" && detail.InvoiceSquence != 7 && detail.InvoiceSquence != 8 && detail.InvoiceSquence != 22 && detail.InvoiceSquence != 23)//中山模式公费显示记账部分
                        {
