...
namespace Neusoft.HISFC.Components.OutpatientFee.Class
{
    public class Function
    {
            ...
            string realInvoiceNO, string invoiceFlag, string splitFlag)
        {
            ...
            if (splitFlag == "0")//广医
            {
                if (invoiceFlag == "2")//记账发票,临时这样写,以后要考虑公费的算法
                {
                    foreach (FeeItemList f in feeItemLists)
                    {
            ...
            string invoiceFlag, ref Balance balance, ref ArrayList balanceLists, string splitType, ref string errText, int countI)
        {
            ...
            if (invoiceType == "2")
            {
                ...
                for (int i = 0; i <= countI; i++)
                {
                    ...
                    int iReturnValue = feeIntegrate.GetInvoiceNOWithHosCode(employee, "C", ref invoiceNO, ref realInvoiceNO, Neusoft.FrameWork.Management.Connection.Hospital.ID, ref errText);
            ...
            string invoiceBeginNO, string realInvoiceBeginNO, ref string errText, System.Data.IDbTransaction t)
        {
            ...
            const string PUB_INVOICE = "2";//记账发票
            ...
            //#endregion
            #region 公费患者
            if (register.Pact.PayKind.ID == "03")//公费患者,打印3张发票,分为自费,记账,和特殊,生成3张发票
            {
                ArrayList ownFeeItemLists = new ArrayList();//自费
                ArrayList pubFeeItemLists = new ArrayList();//记账
        ...
        /// <returns></returns>
        public static Neusoft.HISFC.Models.Base.PactItemRate PactRate(Neusoft.HISFC.Models.Registration.Register r, Neusoft.HISFC.Models.Fee.Outpatient.FeeItemList f, ref string ErrorMessage)
        {
            ...
            //            if (p == null)
            //            {
            //                ErrorMessage = pactInfoManager.Err;
