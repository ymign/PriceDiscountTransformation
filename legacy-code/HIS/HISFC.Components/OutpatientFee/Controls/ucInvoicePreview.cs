...
namespace Neusoft.HISFC.Components.OutpatientFee.Controls
{
    public partial class ucInvoicePreview : UserControl, Neusoft.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientOtherInfomationLeft
    {
        ...
        public int InitInvoice()
        {
            ...
            int iReturn = this.feeIntegrate.GetInvoiceNOWithHosCode(oper, "C", ref invoiceNO, ref realInvoiceNO, Neusoft.FrameWork.Management.Connection.Hospital.ID, ref errText);
            //if (iReturn == -1)
            //{
            ...
            if (this.isValidFee && (string.IsNullOrEmpty(realInvoiceNO) || this.isFirst))
            {
                ...
                iReturn = this.feeIntegrate.GetInvoiceNOWithHosCode(oper, "C", ref invoiceNO, ref realInvoiceNO,Neusoft.FrameWork.Management.Connection.Hospital.ID, ref errText);
                if (iReturn == -1)
                {
                    MessageBox.Show(errText);
                    return -1;
        ...
        public int UpdateInvoice(string invoiceNO,string realInvoiceNO,ref string errInfo)
        {
            ...
            int iRes = feeIntegrate.UpdateNextInvoiceNOWithHosCode(oper.ID, "C", invoiceNO, realInvoiceNO, Neusoft.FrameWork.Management.Connection.Hospital.ID);
            if (iRes <= 0)
            {
