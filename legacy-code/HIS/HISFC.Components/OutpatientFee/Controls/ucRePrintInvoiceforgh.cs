...
namespace Neusoft.HISFC.Components.OutpatientFee.Controls
{
    ...
    /// 发票重打
    /// </summary>
    public partial class ucRePrintInvoiceforgh : Neusoft.FrameWork.WinForms.Controls.ucBaseControl, Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer, IReprintInvoice
    {
        #region 成员
        /// <summary>
        /// 重打发票是否走号
        ...
        public override void ToolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            {
                case "换开纸质票":
                    //if (MessageBox.Show("是否要要将电子票换开纸质票？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    //{
                    ...
                    //    }
                    //    //调用门诊收费换开纸质票接口
                    //    if (useElecBillFee.GetOutPatientFeePaperBill(invoiceNo, operID, ref msg) == -1)
                    //    {
                    //        MessageBox.Show(msg);
                    //        this.Print(1);
        ...
        protected void QueryBalances(string invoiceNo, out string errMsg)
        {
            ...
            DataSet dsghInfo = new DataSet();
            int result = outpatientManager.QueryGHInfoByInvoiceNO(invoiceNo,ref dsghInfo);
            dtghInfo = dsghInfo.Tables[0];
            if (dtghInfo == null)
            {
            ...
            decimal totCost = 0, ownCost = 0, payCost = 0, pubCost = 0;
                ...
                totCost += decimal.Parse(dtghInfo.Rows[0]["TOT_COST"].ToString());
                ownCost += decimal.Parse(dtghInfo.Rows[0]["OWN_COST"].ToString());
                payCost += decimal.Parse(dtghInfo.Rows[0]["PAY_COST"].ToString());
                pubCost += decimal.Parse(dtghInfo.Rows[0]["PUB_COST"].ToString());


                this.tbInvoiceNo.Text = dtghInfo.Rows[0]["INVOICE_NO"].ToString();
                this.tbPName.Text = dtghInfo.Rows[0]["NAME"].ToString();

                Neusoft.HISFC.Models.Base.Employee employee = this.managerIntegrate.GetEmployeeInfo(dtghInfo.Rows[0]["OPER_CODE"].ToString());
            if (employee == null)
            {
                errMsg = "获得当前发票操作员信息失败!" + this.managerIntegrate.Err;
                //MessageBox.Show("获得当前发票操作员信息失败!" + this.managerIntegrate.Err);
            }
            Neusoft.HISFC.Models.Registration.Register regobj = this.registerManager.GetByClinic(dtghInfo.Rows[0]["CLINIC_NO"].ToString());
            ...
            this.tbPactInfo.Text = regobj.Pact.Name;
            ...
            this.tbInvoiceDate.Text = dtghInfo.Rows[0]["OPER_DATE"].ToString();

            if (!FillBalanceLists(dtghInfo))
            {
        ...
        protected void PrintMedicalBanlance()
        {
            ...
            GDSI.ZhuHaiSI.Print.ucNewXQMZBalanceInfo uc = new GDSI.ZhuHaiSI.Print.ucNewXQMZBalanceInfo();
            if (uc.Init(invoiceNo) <= 0)
            {
        ...
        private void PrintOutPatientGuide(DataTable dt)
        {
            #region 门诊指引单打印
        ...
        /// <returns>成功 true 失败 false</returns>
        private bool FillBalanceLists(DataTable dtinfo)
        {
            ...
            decimal pubCost = decimal.Parse(dtinfo.Rows[0]["PUB_COST"].ToString());
                ...
                this.fpSpread1_Sheet1.Cells[0, 0].Text = "诊查费";
                this.fpSpread1_Sheet1.Cells[0, 1].Text = ownCost.ToString();
                this.fpSpread1_Sheet1.Cells[0, 2].Text = payCost.ToString();
                this.fpSpread1_Sheet1.Cells[0, 3].Text = pubCost.ToString();
                this.fpSpread1_Sheet1.Cells[0, 4].Text = (ownCost + payCost + pubCost).ToString();
        ...
        /// 打印门诊指引单
        /// </summary>
        private void PrintGuide(DataTable dt)
        {
        ...
        private void tbID_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            string sql = @"SELECT f.invoice_no,f.print_invoiceno,f.tot_cost,f.oper_date,p.name FROM fin_opb_accountcardfee f,com_patientinfo p 
WHERE 
f.card_no = p.card_no
AND f.cancel_flag = '1'
AND f.fee_type = '4'
AND (f.card_no = '{0}' OR p.name = '{0}')
