...
namespace Neusoft.HISFC.Components.Terminal.Confirm
{
    ...
    ///  />
    /// </summary>
    public partial class ucCancelOutpatientConfirmCancel : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// </summary>
            //Neusoft.HISFC.BizLogic.Terminal.TerminalConfirm confirmIntegrate = new TerminalConfirm();
            // 业务层
            ...
            foreach (FarPoint.Win.Spread.Row r in this.neuSpread2_Sheet1.Rows)
            {
                ...
                //部分确认暂不考虑-即不允许部分确认
                //直接更新met_tec_ta_detail中的已确认数量，将之前的已确认数量存入extend_field3
                result = confirmIntegrate.UpdateConfirmedFlagCancel(apply.ID, Neusoft.FrameWork.Management.Connection.Operator.ID, "");
                if (result != Result.Success)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
