...
namespace Neusoft.HISFC.Components.Terminal.Confirm
{
	...
	public partial class ucItemApply : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
	{
		...
		public void Init()
		{
            ...
            passWord = (Neusoft.FrameWork.Management.Connection.Operator as Neusoft.HISFC.Models.Base.Employee).Password;
            account = (Neusoft.FrameWork.Management.Connection.Operator as Neusoft.HISFC.Models.Base.Employee).User01;
        ...
        public void GetApplyByRegister(bool boolClear, bool boolCardNo, bool boolOther)
		{
            ...
            foreach (Neusoft.HISFC.Models.Terminal.TerminalApply apply in alApplyListFill)
            {
                ...
                // 项目单价2
                this.Cell(0, (int)DisplayField.ItemPrice, Neusoft.FrameWork.Public.String.FormatNumberReturnString(apply.Item.Item.Price, 4));
                // 已确认数9
                ...
                if (!string.IsNullOrEmpty(apply.Item.UndrugComb.ID))
                {
                    if (packageTotCost.ContainsKey(apply.Item.UndrugComb.ID + apply.Order.ID))
                    {
                        this.Cell(0, (int)DisplayField.ItemTotalPrice,
                            Neusoft.FrameWork.Public.String.FormatNumberReturnString(
			...
			for (int i = 0;i < this.fpSpread1_Sheet1.RowCount;i++)
			{
                ...
                Neusoft.HISFC.Models.Terminal.TerminalApply apply = this.fpSpread1_Sheet1.Rows[i].Tag as Neusoft.HISFC.Models.Terminal.TerminalApply;
		...
		public int DataValidate()
		{
			...
			for (int i = 0;i < this.RowCount;i++)
			{
				...
				if (this.GetItem(i, (int)DisplayField.NewOld) == "旧")
				{
                    ...
                    decimal itemSum = Neusoft.FrameWork.Function.NConvert.ToDecimal(fpSpread1_Sheet1.Cells[i, (int)DisplayField.ItemPrice].Text) * objTemp.Item.Item.Qty;
                    if (Neusoft.FrameWork.Function.NConvert.ToDecimal(fpSpread1_Sheet1.Cells[i, (int)DisplayField.ItemCost].Text) !=  Neusoft.FrameWork.Public.String.FormatNumber(itemSum,2))
                    {
                        //判断当前已确认数量是否发生变化
        ...
        private void ShowPacsApply(int rowIndex)
        {
            ...
            if (!string.IsNullOrEmpty(isUseDL) && isUseDL == "1")
            {
                ...
                if (this.GetItem(rowIndex, (int)DisplayField.NewOld) == "旧")
                {
                    ...
                    Neusoft.HISFC.Models.Order.Order order = orderIntegrate.GetOneOrder(apply.Patient.ID,apply.Order.ID);
                    if (order == null)
                    {
                    ...
                    {
                        if (hisInterface.UpdateApply(order.ApplyNo,false) < 0)
                        {
