...
namespace Neusoft.HISFC.Components.InpatientFee.Register
{
    ...
    public partial class ucUnregisterNoFee : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        protected int LeaveHospital()
        {
            ...
            //检索是否有转入预交金--出院结算处理
            Neusoft.HISFC.Models.Fee.Inpatient.Prepay[] Prepays = (Neusoft.HISFC.Models.Fee.Inpatient.Prepay[])al.ToArray(typeof(Neusoft.HISFC.Models.Fee.Inpatient.Prepay));
            List<Neusoft.HISFC.Models.Fee.Inpatient.Prepay> listPrepay = new List<Neusoft.HISFC.Models.Fee.Inpatient.Prepay>(Prepays);
            #region 调用信用就医出院接口
            if (listPrepay.Where(x => x.PayType.ID == "YBXYF").ToList().Count > 0)
            {
                try
                {
                    HisCallExternalServiceProject.ZDWYPayPlatform.ZFPTService ZFPTSer = new HisCallExternalServiceProject.ZDWYPayPlatform.ZFPTService();
                    HisCallExternalServiceProject.ZDWYPayPlatform.Models.ReqLeaveHospital ZFPTrqModel = new HisCallExternalServiceProject.ZDWYPayPlatform.Models.ReqLeaveHospital();
                    HisCallExternalServiceProject.ZDWYPayPlatform.Models.Response<HisCallExternalServiceProject.ZDWYPayPlatform.Models.ResLeaveHospital> ZFPTresponseModel = new HisCallExternalServiceProject.ZDWYPayPlatform.Models.Response<HisCallExternalServiceProject.ZDWYPayPlatform.Models.ResLeaveHospital>();
                    ZFPTrqModel.HospitalSerialNo = patientInfo.ID;//住院流水号
                    ZFPTrqModel.PatientCard = patientInfo.IDCard;
                    ZFPTrqModel.PatientName = patientInfo.Name;
                    ZFPTrqModel.CardType = "1";
                    ZFPTrqModel.OperatorNo = this.feeInpatient.Operator.ID;
                    if (ZFPTSer.LeaveHospital(ZFPTrqModel, ref ZFPTresponseModel) != 1)
                    {
                        CommonController.Instance.MessageBox("调用信用就医出院接口出错：" + ZFPTSer.ErrMsg, MessageBoxIcon.Exclamation);
                    }
                    if (ZFPTresponseModel.Code != "1")
                    {
                        CommonController.Instance.MessageBox("调用信用就医出院接口出错：" + ZFPTresponseModel.Msg, MessageBoxIcon.Exclamation);
                    }
                }
                catch (Exception ex)
                {
                    CommonController.Instance.MessageBox("调用信用就医出院接口出错：" + ex.Message, MessageBoxIcon.Exclamation);
                }
            }
            #endregion
            return 1;
        ...
        protected virtual void Confirm()
        {
            ...
            if (isRecycle)
            {
                ...
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(Neusoft.FrameWork.Management.Language.Msg("回收住院号错误！") + this.radtIntegrate.Err);
                    return;
                }
            }
