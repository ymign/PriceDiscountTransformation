...
namespace Neusoft.HISFC.Components.InpatientFee.Maintenance
{
    ...
    public partial class ucStopAndBegin : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 日志业务层
        /// </summary>
        Neusoft.HISFC.BizLogic.RADT.InPatient radtMgr = new Neusoft.HISFC.BizLogic.RADT.InPatient();
        ...
        private int ProcessAcountFlag(bool isStopAccount)
        {
            ...
            if (isStopAccount)
            {
                ...
                Neusoft.FrameWork.Models.NeuObject objOpen = new Neusoft.FrameWork.Models.NeuObject("0","开账","");
                Neusoft.FrameWork.Models.NeuObject objClose = new Neusoft.FrameWork.Models.NeuObject("1","关账","");
                if (radtMgr.SetShiftData(p.ID, Neusoft.HISFC.Models.Base.EnumShiftType.AC, "手工", objOpen, objClose, p.IsBaby) < 0)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(Language.Msg("封账失败") + this.inPatientFee.Err);
                    return -1;
            ...
            else
            {
                ...
                Neusoft.FrameWork.Models.NeuObject objOpen = new Neusoft.FrameWork.Models.NeuObject("0", "开账", "");
                Neusoft.FrameWork.Models.NeuObject objClose = new Neusoft.FrameWork.Models.NeuObject("1", "关账", "");
                if (radtMgr.SetShiftData(p.ID, Neusoft.HISFC.Models.Base.EnumShiftType.AO, "手工", objClose, objOpen, p.IsBaby) < 0)
                {
                    Neusoft.FrameWork.Management.PublicTrans.RollBack();
                    MessageBox.Show(Language.Msg("开账失败") + this.inPatientFee.Err);
                    return -1;
