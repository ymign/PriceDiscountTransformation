...
{
    public partial class ucNewHCCompare : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        public ucNewHCCompare()
        {
            InitializeComponent();
        }
        ...
        private void InitColumn()
        {
            ...
            Type date = typeof(System.DateTime);
                ...
                DataColumn[] colCenterItem = new DataColumn[]{new DataColumn("医保耗材代码", str),
                                                                 new DataColumn("一级分类", str),
                                                                 new DataColumn("二级分类", str),
                                                                 new DataColumn("三级分类", str),
                                                                 new DataColumn("医保通用名", str),
                                                                 new DataColumn("材质", str),
                                                                 new DataColumn("特征", str),
                                                                 new DataColumn("注册证号", str),
                                                                 new DataColumn("单件产品名称", str),
                                                                new DataColumn("耗材企业", str),
                                                                new DataColumn("规格型号数", str)};
                dtCenterItem.Columns.AddRange(colCenterItem);
                DataColumn[] keyCenter = new DataColumn[11];
                keyCenter[0] = dtCenterItem.Columns[0];
                keyCenter[1] = dtCenterItem.Columns[1];
                keyCenter[2] = dtCenterItem.Columns[2];
                keyCenter[3] = dtCenterItem.Columns[3];
                keyCenter[4] = dtCenterItem.Columns[4];
                keyCenter[5] = dtCenterItem.Columns[5];
                keyCenter[6] = dtCenterItem.Columns[6];
                keyCenter[7] = dtCenterItem.Columns[7];
                keyCenter[8] = dtCenterItem.Columns[8];
                keyCenter[9] = dtCenterItem.Columns[9];
                keyCenter[10] = dtCenterItem.Columns[10];
                dtCenterItem.CaseSensitive=true;
                dtCenterItem.PrimaryKey = keyCenter;
                dvCenterItem = new DataView(dtCenterItem);
                dvCenterItem.Sort = "医保耗材代码 ASC";
        ...
        public void InitData()
        {
            ...
            if (!string.IsNullOrEmpty(this.DefaulPactCode))
            {
                ...
                #region 加载非药品
            alHisItem = myInterface.GetNoCompareHCItem("16", compareUndrugType.ToString());
                if (alHisItem != null)
                {
                ...
                this.myInterface.GetGBHCITEM(ref dtCenterItem);

                this.myInterface.GetHCCompareItem(ref dtCompareItem);
        ...
        private void FilterItem(string flag, string input)
        {
            ...
            switch (flag)
            {
                ...
                case "CENTER":
                    if (!this.checkBox1.Checked)
                    {
                        filterString = "医保耗材代码" + " like '" + input + "%'" + " or " + " 单件产品名称" + " like '" + input + "%'";
                    }
                    else
                    {
                        filterString = "医保耗材代码" + " like '%" + input + "%'" + " or " + " 单件产品名称" + " like '%" + input + "%'";
                ...
                case "COMPARE":
                    if (!this.checkBox1.Checked)
                    {
                        filterString = "非药品编码" + " like '" + input + "%'" + " or " + " 非药品名称" + " like '" + input + "%'" + " or " + " 单件产品名称" + " like '%" + input + "%'";
                    }
                    else
                    {
                        filterString = "非药品编码" + " like '%" + input + "%'" + " or " + " 非药品名称" + " like '%" + input + "%'" + " or " + " 单件产品名称" + " like '%" + input + "%'";
        ...
        private void SetCenterItemInfo(int row)
        {
            ...
            Name = this.fpCenterItem_Sheet1.Cells[row, 8].Text.Trim();
            Item obj = this.GetSelectCenterItem(centerCode,Name);
        ...
        /// <returns></returns>
        private Neusoft.HISFC.Models.SIInterface.Item GetSelectCenterItem(string centerCode,string name )
        {
            Item obj = new Item();
        ...
        public void Compare()
        {
            Compare objCom = new Compare();
                ...
                row["医保耗材代码"] = objCenter.ID;
                row["单件产品名称"] = objCenter.Name;
