...
namespace Neusoft.HISFC.Components.InpatientFee.Maintenance
{
    public partial class ucCompare : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        public void InitData()
        {
            ...
            ArrayList itemGrade = consMgr.GetAllList("GTGRADE");
            ...
            if (isDrug)
            {
                ...
                if (alCompareItem != null)
                {
                    foreach (Neusoft.HISFC.Models.SIInterface.Compare obj in alCompareItem)
                    {
                        ...
                        row["医保目录等级"] = string.IsNullOrEmpty(obj.CenterItem.ItemGrade)?"未对照":itemGradeHelper.GetObjectFromID(obj.CenterItem.ItemGrade).Name;
                        //switch (obj.CenterItem.ItemGrade)
                        //{
                        //    case "1":
                        //        row["医保目录等级"] = "基本类";
                        //        break;
            ...
            {
                #region 加载非药品
                alHisItem = myInterface.GetNoCompareUndrugItem(pactCode.ID,compareUndrugType.ToString());
                if (alHisItem != null)
                {
