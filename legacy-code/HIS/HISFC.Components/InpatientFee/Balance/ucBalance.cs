...
namespace Neusoft.HISFC.Components.InpatientFee.Balance
{
    ...
    public partial class ucBalance : Neusoft.FrameWork.WinForms.Controls.ucBaseControl, Neusoft.HISFC.BizProcess.Interface.FeeInterface.ISIReadCard, Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        private Neusoft.HISFC.BizProcess.Integrate.Common.ControlParam controlParm = new Neusoft.HISFC.BizProcess.Integrate.Common.ControlParam();
        ...
        */
        ...
        protected virtual int QueryFeeInfo(DateTime dtFeeBegin, DateTime dtFeeEnd)
        {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                //获取最小费用名称
                feeInfo.Item.MinFee.Name = Neusoft.SOC.HISFC.BizProcess.CommonInterface.CommonController.CreateInstance().GetConstantName(EnumConstant.MINFEE, feeInfo.Item.MinFee.ID);
        ...
        protected int ReadControlInfo()
        {
            ...
            try
            {
                ...
                //是否打印婴儿发票
                this.IsBabyPrint = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100004", isRefreashContromParam));
                //是否打印减免发票
                this.IsDeratePrint = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100005", isRefreashContromParam));
                //是否打印膳食发票
                this.IsFoodPrint = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100006", isRefreashContromParam));
                //是否打印减免发票
                this.IsRebatePrint = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100007", isRefreashContromParam));
                //是否可输入减免费用


                this.IsInputDerateFee = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100009", isRefreashContromParam));

                //是否处理转押金


                this.IsTransPrepay = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100018", isRefreashContromParam));
                //是否打印自费药发票


                this.IsOwnMediPrint = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100026", isRefreashContromParam));
                //支票汇票是否需要输入完整银行信息


                this.IsFullBankInfo = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100025", isRefreashContromParam));

                IsModifyCost = Neusoft.FrameWork.Function.NConvert.ToBoolean(controlParm.GetControlParam<bool>("100023", isRefreashContromParam));
        ...
        protected virtual int UpdateInhos(int balNo, DateTime dtBalance)
        {
            ...
            //{
                ...
                if (babyList != null && babyList.Count > 0) 
                {
                    ...
                    {
                        //婴儿取消登记状态
                        if (p.PVisit.InState.ID.ToString() == Neusoft.HISFC.Models.Base.EnumInState.C.ToString())
                        {
                            continue;
                        }
