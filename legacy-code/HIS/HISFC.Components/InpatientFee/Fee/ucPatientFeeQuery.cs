...
namespace Neusoft.HISFC.Components.InpatientFee.Fee
{
    public partial class ucPatientFeeQuery : Neusoft.FrameWork.WinForms.Controls.ucBaseControl
    {
        ...
        /// 其它出院诊断表
        /// </summary>
        DataTable dtOutDiagInfo = new DataTable();
        /// <summary>
        /// 其它出院诊断视图
        /// </summary>
        DataView dvOutDiagInfo = new DataView();
        #endregion
        ...
        private void QueryPatientDrugList(DateTime beginTime, DateTime endTime)
        {
            ...
            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList obj in drugList)
            {
                ...
                if (this.hsDayFeeItemList.ContainsKey(obj.Item.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")) == false)
                {
                    hsDayFeeItemList[obj.Item.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")] = new ArrayList();
                }
                ((ArrayList)hsDayFeeItemList[obj.Item.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")]).Add(obj);
        ...
        private void QueryPatientUndrugList(DateTime beginTime, DateTime endTime)
        {
            ...
            //物价系统兼容时间取数逻辑
            ArrayList undrugList = this.feeManager.QueryFeeItemListsBecauseWJ(this.currentPatient.ID, beginTime, endTime);
            if (undrugList == null)
            {
            ...
            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList obj in undrugList)
            {
                ...
                if (this.IsPackageItem(obj))
                {
                    ...
                    if (this.hsDayFeeItemList.ContainsKey(obj.UndrugComb.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")) == false)
                    {
                        hsDayFeeItemList[obj.UndrugComb.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")] = new ArrayList();
                    }
                    ((ArrayList)hsDayFeeItemList[obj.UndrugComb.ID + obj.ExecOrder.DateUse.ToString("yyyy-MM-dd")]).Add(obj);
                    #endregion
                }
                else
                {
        ...
        private void QueryPatientFeeInfo(DateTime beginTime, DateTime endTime)
        {
            ...
            //住院处要求:接单界面需要增加费别汇总栏优先排序功能（床位费，诊查费，护理费，手术费） by yhm 2024-02-28
            Neusoft.HISFC.BizLogic.Fee.FeeCodeStat feeCodeStat = new Neusoft.HISFC.BizLogic.Fee.FeeCodeStat();
            ArrayList alFeeState = feeCodeStat.QueryFeeCodeStatByReportCode("ZY01");
            if (alFeeState == null)
            {
                MessageBox.Show("获取费用大类出现错误" + feeCodeStat.Err);
                return;
            }

            //床位费 12    诊查费 04  护理费 09   手术费 08
            var feeCodeStatDict = alFeeState.Cast<FeeCodeStat>().ToDictionary(w => w.MinFee.ID);
            var feeTypeOrder = new Dictionary<string, int> { { "12", 1 }, { "04", 2 }, { "09", 3 }, { "08", 4 } };

            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeInfo feeInfo in feeInfoList)
            {
                feeInfo.UpdateSequence = 5; // 其他类别都排在四种费用之后
                if (feeCodeStatDict.ContainsKey(feeInfo.Item.MinFee.ID))
                {
                    var feeCodeStatInfo = feeCodeStatDict[feeInfo.Item.MinFee.ID];
                    if (feeTypeOrder.ContainsKey(feeCodeStatInfo.StatCate.ID))
                    {
                        feeInfo.UpdateSequence = feeTypeOrder[feeCodeStatInfo.StatCate.ID];
                    }
                }
            }

            var feeList = feeInfoList.Cast<FeeInfo>().OrderBy(p => p.UpdateSequence).ToList();


            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeInfo feeInfo in feeList)
            {
        ...
        /// <summary>
        /// 查询其它出院诊断信息
        /// （使用了dblink，所以查询时开启了事务，查询后回滚事务）
        /// </summary>
        /// <param name="patient"></param>
        private void QueryOutDiagInfo(PatientInfo patient)
        {
            string err = "";
            try
            {
                Neusoft.FrameWork.Management.PublicTrans.BeginTransaction();
                this.diagMgr.SetTrans(Neusoft.FrameWork.Management.PublicTrans.Trans);

                var diaglist = this.diagMgr.QueryOtherOutDiagnoseFromEmrView(patient.ID);
                if (diaglist == null)
                {
                    err = Language.Msg("获得患者其它出院诊断信息出错!") + this.diagMgr.Err;
                }
                else
                {
                    foreach (var diag in diaglist)
                    {
                        DataRow row = this.dtOutDiagInfo.NewRow();
                        row["诊断名称"] = diag.Name;
                        row["诊断编码"] = diag.ID;

                        dtOutDiagInfo.Rows.Add(row);
                    }
                    this.fpOutDiagInfo_Sheet1.Columns[0].Width = this.fpOutDiagInfo_Sheet1.Columns[0].GetPreferredWidth();
                }
            }
            catch (Exception ex)
            {
                err = Language.Msg("获得患者其它出院诊断信息出错!") + ex.Message;
            }
            finally
            {
        ...
        public int AddFeeDetailToDataTable(Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList feeItemList)
        {
            ...
            dr["国家医保码"] = feeManager.GetCenterCodeByHIS_CODE(this.GetItemID(feeItemList));
        ...
        private int InitDataTable()
        {
            ...
            #region 其它出院诊断信息
            dtOutDiagInfo.Columns.AddRange(new DataColumn[]{new DataColumn("诊断名称", str),
															new DataColumn("诊断编码", str)});

            dvOutDiagInfo = new DataView(dtOutDiagInfo);
            this.fpOutDiagInfo_Sheet1.DataSource = dvOutDiagInfo;
            #endregion
        ...
        protected void SetPatientInfo()
        {
            ...
            if (al.Count == 0)
            {
                ...
                //医保预结算的信息要先隐藏掉
                this.lblSITotCost.Visible = false;
                this.lblSIPubCost.Visible = false;
                this.lblSIOwnCost.Visible = false;
                this.lblSIFreeCost.Visible = false;
            }
            else
            {
                ...
                this.lblSITotCost.Visible = true;
                this.lblSIPubCost.Visible = true;
                this.lblSIOwnCost.Visible = true;
                this.lblSIFreeCost.Visible = true;
                ...
                this.lblSITotCost.Text = this.myPatientInfoTemp.SIMainInfo.TotCost.ToString();//预结算时总金额
                this.lblSIPubCost.Text = this.myPatientInfoTemp.SIMainInfo.PubCost.ToString();//预结算时报销金额
                this.lblSIOwnCost.Text = this.myPatientInfoTemp.SIMainInfo.OwnCost.ToString();//预结算时个人自付金额
                this.lblSIFreeCost.Text = (this.currentPatient.FT.PrepayCost - this.myPatientInfoTemp.SIMainInfo.OwnCost).ToString();//剩余金额
            ...
            if ((this.currentPatient.FT.PrepayCost + this.currentPatient.FT.BalancedCost - this.myPatientInfoTemp.FT.OwnCost) < 0)//剩余金额少于0，字体变为红色
            {
                ...
                this.lblFreeCost.Text = (this.currentPatient.FT.PrepayCost + this.currentPatient.FT.BalancedCost - this.myPatientInfoTemp.FT.OwnCost).ToString();
            ...
            //catch (Exception ex) { }
            //取出院诊断
            this.lblOutDiagInfo.Text = this.diagMgr.ExecSqlReturnOne(string.Format("select diagn1_name from fin_ipr_leavenotic where inpatient_no='{0}' ", this.currentPatient.ID), "");
            this.lblDlagn3Name.Text = this.diagMgr.ExecSqlReturnOne(string.Format("select DIAGN3_NAME from fin_ipr_leavenotic where inpatient_no='{0}' ", this.currentPatient.ID), "");
            this.lblDlagn3Icd.Text = this.diagMgr.ExecSqlReturnOne(string.Format("select DIAGN3_ICD from fin_ipr_leavenotic where inpatient_no='{0}' ", this.currentPatient.ID), "");
            string DayOperationFlag = this.diagMgr.ExecSqlReturnOne(string.Format("select DAY_OPERATION from fin_ipr_leavenotic where inpatient_no='{0}' ", this.currentPatient.ID), "");
            if (this.currentPatient.DayOperationFlag == "1")
            {
                this.lblDayOpertionFlag.Text = "日间手术";
            }
            else if (this.currentPatient.DayOperationFlag == "2")
            {
                this.lblDayOpertionFlag.Text = "日间化疗";
            }
            else if (this.currentPatient.DayOperationFlag == "3")
            {
                this.lblDayOpertionFlag.Text = "预入院";
            }
            else
            {
                this.lblDayOpertionFlag.Text = "否";
            }
            if (DayOperationFlag == "1")
            {
                this.lblDayoperation.Text = "日间手术";
            }
            else if (DayOperationFlag == "2")
            {
                this.lblDayoperation.Text = "日间化疗";
            }
            else if (DayOperationFlag == "3")
            {
                this.lblDayoperation.Text = "预入院";
            }
            else
            {
                this.lblDayoperation.Text = "否";
            }
        }
        ...
        protected void QueryAllInfomaition(DateTime beginTime, DateTime endTime)
        {
            ...
            if (this.neuTabControl1.TabPages.Contains(this.tpMainInfo))
            {
                ...
                this.QueryOutDiagInfo(this.currentPatient);
            }
        ...
        public void Clear()
        {
            ...
            this.lblDiagnose.Text = this.RetrunSplit(20);  //入院诊断
            this.lblMemo.Text = this.RetrunSplit(30);
            this.txtArriveDate.Text = this.RetrunSplit(20);
            this.txtDays.Text = this.RetrunSplit(3);
            this.lblDayoperation.Text = this.RetrunSplit(8); //出院结算类型
            this.lblDlagn3Name.Text = this.RetrunSplit(17);//手术名称
            this.lblDlagn3Icd.Text = this.RetrunSplit(13);//手术编码
            //this.lblInDiagInfo.Text = this.RetrunSplit(30);  //在院诊断
            this.lblOutDiagInfo.Text = this.RetrunSplit(30); //出院诊断
        ...
        private void fpFee_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("费用查询中...", 0, false);
                    Application.DoEvents();

                    hsMinFee[feeCode] = new ArrayList();
                    var undrugList = this.feeManager.QueryFeeItemListsNew(this.currentPatient.ID, feeCode);
                    var drugList = this.feeManager.GetMedItemsForInpatientNew(this.currentPatient.ID, feeCode);
                    ...
                    foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList obj in undrugList)
                    {
                        ...
                        ((ArrayList)hsMinFee[feeCode]).Add(obj);
                    }
                    ...
                    this.alAllFeeItemLists = undrugList;
                    this.alAllFeeItemLists.AddRange(drugList);
                }
                catch (Exception)
                {
            ...
            for (int i = alAllFeeItemLists.Count - 1; i >= 0; i--)
            {
                ...
                if (!hsItemsItemSum.Contains(itemCode))
                {
                    ...
                    itemSum.FeeOper.OperTime = item.FeeOper.OperTime;
                    itemSum.ExecOrder.ID = item.ExecOrder.ID;
                    itemSum.ExecOrder.DateUse = item.ExecOrder.DateUse;
        ...
        private void fpFeeItemSum_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            ...
            for (int i = alAllFeeItemLists.Count - 1; i >= 0; i--)
            {
                ...
                //if (!hsItemsDaySum.Contains(item.ChargeOper.OperTime.ToString("yyyy-MM-dd") + itemCode))//收费时间
                if (!hsItemsDaySum.Contains(item.ExecOrder.DateUse.ToString("yyyy-MM-dd") + itemCode))//执行时间
                {
                    Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList itemSum = new Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList();
                    ...
                    itemSum.ExecOrder.ID = item.ExecOrder.ID;
                    itemSum.UndrugComb = item.UndrugComb.Clone();
                    itemSum.ExecOrder.DateUse = itemSum.ExecOrder.DateUse;
                    ...
                    itemSum.FT.OwnCost = item.FT.OwnCost;
                    itemSum.FT.PubCost = item.FT.PubCost;
                    //hsItemsDaySum.Add(itemSum.ChargeOper.OperTime.ToString("yyyy-MM-dd") + itemSum.Item.ID, itemSum);//收费时间
                    hsItemsDaySum.Add(itemSum.ExecOrder.DateUse.ToString("yyyy-MM-dd") + itemSum.Item.ID, itemSum);//执行时间
                }
                else
                {

                    Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList itemSum = hsItemsDaySum[item.ExecOrder.DateUse.ToString("yyyy-MM-dd") + itemCode] as Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList;
                    if (this.IsPackageItem(item))
                    {
                        string key = this.GetPackageItemKey(item);
                        if (packageCollection.ContainsKey(key) == false && item.UndrugComb.Qty != 0)
            ...
            foreach (System.Collections.DictionaryEntry objOrder in hsItemsDaySum)
            {
                ...
                dr["执行时间"] = itemSum.ExecOrder.DateUse.ToString("yyyy-MM-dd");//财务科毛凤飞要求改为执行时间
        ...
        private void fpFeeDaySum_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            ...
            this.dtFeeDetail.Clear();
            string feeCode = this.fpFeeDaySum_Sheet1.Cells[this.fpFeeDaySum_Sheet1.ActiveRowIndex, 4].Text;//4
            string feeName = this.fpFeeDaySum_Sheet1.Cells[this.fpFeeDaySum_Sheet1.ActiveRowIndex, 1].Text;//1
            ...
            string feeUseDate = this.fpFeeDaySum_Sheet1.Cells[this.fpFeeDaySum_Sheet1.ActiveRowIndex, 0].Text;
            ...
            if (this.hsDayFeeItemList.ContainsKey(feeCode + feeUseDate))
            {
                this.alAllFeeItemLists = this.hsDayFeeItemList[feeCode + feeUseDate] as ArrayList;
            }
            ...
            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList item in this.alAllFeeItemLists)
            {
                ...
                //if (this.GetItemID(item)== feeCode && item.ChargeOper.OperTime.ToString("yyyy-MM-dd") == Neusoft.FrameWork.Function.NConvert.ToDateTime(feeDate).ToString("yyyy-MM-dd"))
                if (this.GetItemID(item) == feeCode && item.ExecOrder.DateUse.ToString("yyyy-MM-dd") == Neusoft.FrameWork.Function.NConvert.ToDateTime(feeUseDate).ToString("yyyy-MM-dd"))
                {
                    //重取数据库
        ...
        private void ucQueryInpatientNo1_myEvent()
        {
            ...
            this.QueryPatientByInpatientNO(this.currentPatient);
        ...
        private void dtpItemDayFilter_ValueChanged(object sender, EventArgs e)
        {
            if (this.dvFeeDaySum != null)
            {
                if (this.dtpItemDayFilter.Checked)
                {
                    ...
                    this.dvFeeDaySum.RowFilter = string.Format("执行时间 ='{0}'", this.dtpItemDayFilter.Value.ToString("yyyy-MM-dd"));
                }
                else
                {
                    this.dvFeeDaySum.RowFilter = "1=1";
        ...
        //医保预结算 {AC1CBAC0-6E11-4d1f-AF4B-049B40010288}
        private void button1_Click(object sender, EventArgs e)
        {
            //if (this.currentPatient.Pact.PayKind.ID != "02" || this.currentPatient.Pact.ID == "84" || this.currentPatient.Pact.ID == "251")
            //{
            //    MessageBox.Show(Language.Msg("医保预结算只支持医保患者且是本市医保患者"));
            //    return;
            //}
            if (this.currentPatient.Pact.ID != "115")
            {
                MessageBox.Show(Language.Msg("只支持广州医保本市普通住院的患者进行预结算！"));
                return;
            }

            try
            {
                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("正在进行医保预结算，请稍后...");
                Application.DoEvents();
                GDSI.Business.ServiceLogic SIServer = new GDSI.Business.ServiceLogic();
                System.Collections.ArrayList FeeDetails = new System.Collections.ArrayList();
                GDSI.Models.OutParam.OutParamBizh120003 OutParam = new GDSI.Models.OutParam.OutParamBizh120003();

                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("撤销医保费用中...");
                //先删除费用明细
                SIServer.DeleteUploadedFeeDetailsInpatient(currentPatient);

                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("上传医保费用中...");
                //费用上传
                if (SIServer.UploadFeeDetailsInpatientForIN(currentPatient, ref FeeDetails) != 1)
                {
                    Neusoft.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    MessageBox.Show(Language.Msg(SIServer.ErrMsg));
                    return;
                }

                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("医保预结算中...");
                //预结算
                if (SIServer.PreBalanceInpatient(currentPatient, ref FeeDetails, ref OutParam) != 1)
                {
                    Neusoft.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    MessageBox.Show(Language.Msg(SIServer.ErrMsg));
                    return;
                }
                Neusoft.FrameWork.WinForms.Classes.Function.ShowWaitForm("数据重新加载中...");
                //更新费用控制临时表
                SIServer.UpdateSIControlTemp(currentPatient, "1", "");

                //更新住院主表
                SIServer.UpdateInmaininfoOwnCost(currentPatient);

                this.currentPatient = this.radtManager.QueryPatientInfoByInpatientNO(currentPatient.ID);
                if (this.currentPatient == null || this.currentPatient.ID == null || this.currentPatient.ID == string.Empty)
                {
                    Neusoft.FrameWork.WinForms.Classes.Function.HideWaitForm();
                    MessageBox.Show(Language.Msg("查询患者基本信息出错!") + this.radtManager.Err);
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("预结算一次：" + ex.Message);
            }
            finally
            {
                Neusoft.FrameWork.WinForms.Classes.Function.HideWaitForm();
        ...
        /// 外部设置住院患者信息
        /// </summary>
        /// <param name="pInfo">患者信息实体</param>
        public void SetInpatientInfo(Neusoft.HISFC.Models.RADT.PatientInfo pInfo)
        {
            if (pInfo == null) return;
            this.ucQueryInpatientNo1.Text = pInfo.PID.PatientNO;
            this.currentPatient = pInfo;

            this.QueryPatientByInpatientNO(this.currentPatient);
        }
        /// <summary>
        /// 用于自助出院代办界面时，设置一些控件的属性
        /// </summary>
        /// <returns></returns>
        public void SetControlForSelfOut()
        {
            this.ucQueryInpatientNo1.TextBox.ReadOnly = true;
        }
