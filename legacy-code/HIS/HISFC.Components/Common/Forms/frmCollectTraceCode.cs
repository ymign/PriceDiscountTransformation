...
{
    public partial class frmCollectTraceCode : Form
    {
        #region 属性
        ...
        private bool EnableSplitDrugFixedUniversalCodeUpload = new Neusoft.HISFC.BizProcess.Integrate.Common.ControlParam().GetControlParam<bool>("EnableSplitDrugFixedUniversalCodeUpload", false, false);
        #endregion

        public frmCollectTraceCode()
        {
            InitializeComponent();
        ...
        private void SetStyle()
        {
            ...
            SetButtonStyle(btn对照维护, Color.FromArgb(255, 33, 150, 243), Color.FromArgb(255, 25, 118, 210));
            // 拆零维护
        ...
        public void InitData(List<YbTraceCollectMain> list)
        {
            if (!list.Any())
            {
            ...
            YbTraceCollectMain info)
        {
            ...
            if (info.PharmacyCode != LoginEmployee.Dept.ID)
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + "取药药房与当前登陆科室不一致!");
                return;
            }

            var stockInfo = this.QueryService.GetYbTraceStockInfo(info.DrugCode, info.PharmacyCode);
            if (stockInfo == null)
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + "查找库存信息失败,请确认是否已经进行拆零入库!");
                return;
            }

            if (stockInfo.AvailableQty < info.SplitNeedCollectQty)
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + "拆零可用库存小于当前所需库存,请及时补充!");
                return;
            }
            ...
            var splitTraceCodeList = this.QueryService.GetInStockSplitTraceCodeList(
                info.DrugCode,
                info.PharmacyCode,
                info.SplitNeedCollectQty);

            if (!splitTraceCodeList.Any())
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + this.QueryService.Err);

                return;
            }

            if (splitTraceCodeList.Count != info.SplitNeedCollectQty)
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + "可用拆零库存[" + splitTraceCodeList.Count + "]不足,请及时补充!");

                return;
            }

            if (info.SplitTracCodgsList == null || info.SplitTracCodgsList.Count <= 0)
            {
                info.SplitTracCodgsList = new List<string>();
            }

            foreach (var splitTraceCodeInfo in splitTraceCodeList)
            {
                info.SplitTracCodgsList.Add(splitTraceCodeInfo.ChildTraceCode);
            }
            ...
            //将每一个最新分配的拆零追溯码状态锁定
            var idList = splitTraceCodeList.Select(s => s.Id).ToList();
            if (!this.QueryService.UpdateYbTraceStateRecordTraceState(idList, "5", LoginEmployee.ID, LoginEmployee.Name))
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + this.QueryService.Err);
            ...
            //更新库存表预扣库存与可用库存
            if (!this.QueryService.UpdateYbTraceStockWhenPredeductedSucess(
                info.PharmacyCode,
                info.DrugCode,
                info.SplitNeedCollectQty,
                LoginEmployee.ID, LoginEmployee.Name))
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + this.QueryService.Err);
            ...
            stockRecordInfo.DrugCode = info.DrugCode;
            stockRecordInfo.DrugName = info.DrugName;
            stockRecordInfo.DrugDeptCode = info.PharmacyCode;
            stockRecordInfo.DrugDeptName = info.PharmacyName;
            stockRecordInfo.ChangeType = "1";
            ...
            stockRecordInfo.AfterTotalQty = stockRecordInfo.BeforeTotalQty;
            stockRecordInfo.AfterAvailableQty = stockRecordInfo.BeforeAvailableQty - info.SplitNeedCollectQty;
            stockRecordInfo.AfterPredeductedQty = stockRecordInfo.BeforePredeductedQty + info.SplitNeedCollectQty;
            ...
            stockRecordInfo.RelatedId = info.ApplyNumber;
            stockRecordInfo.RelatedNo = info.ApplyNumber;
            stockRecordInfo.CreatedCode = LoginEmployee.ID;
            stockRecordInfo.CreatedName = LoginEmployee.Name;

            if (!this.QueryService.InsertYbTraceStockRecord(stockRecordInfo))
            {
                SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + this.QueryService.Err);
            ...
            //插入每一个拆零子码的变更记录
            foreach (var stateRecordInfo in splitTraceCodeList)
            {
                var codeRecordInfo = new YbTraceCodeRecord();
                codeRecordInfo.Id = Guid.NewGuid().ToString();
                codeRecordInfo.DrugCode = info.DrugCode;
                codeRecordInfo.DrugName = info.DrugName;
                codeRecordInfo.ParentTraceCode = stateRecordInfo.ParentTraceCode;
                codeRecordInfo.ChildTraceCode = stateRecordInfo.ChildTraceCode;
                codeRecordInfo.SequenceNo = stateRecordInfo.SequenceNo;
                codeRecordInfo.RelatedOrderNo = info.ApplyNumber;
                codeRecordInfo.RelatedTableName = "pha_com_applyout";
                codeRecordInfo.RelatedId = info.ApplyNumber;

                codeRecordInfo.BusinessScenario = info.BusinessScenario;
                codeRecordInfo.OperationType = "1";
                codeRecordInfo.OperationTime = DateTime.Now;
                codeRecordInfo.OperationJson = "";
                codeRecordInfo.OperationDescription = "拆零出库预扣成功,子追溯码:" + codeRecordInfo.ChildTraceCode + "状态变更为占用中!";
                codeRecordInfo.CreatedCode = LoginEmployee.ID;
                codeRecordInfo.CreatedName = LoginEmployee.Name;

                if (!this.QueryService.InsertYbTraceCodeRecord(codeRecordInfo))
                {
                    SetSplitTraceCodeFPErrMsg(rowIndex, errMsg + this.QueryService.Err);
        ...
        private void txtCollectTraceCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                ShowErrorMessage(scanCode, "扫码格式错误，长度不足7位！");
                return;
            }
            ...
            {
                if (item.IdentifiyCodeList.Contains(identifiyCode))
                {
                    drugCode = item.DrugCode;
                    collectMainInfo = item;
                    break;
                }
            }
            ...
            {
                ShowErrorMessage(scanCode, "本次发药品种不包含该标识码,请核对!");
                return;
            }
            ...
            if (!TraceCodeCollectionStatusEnum.IsValid(collectMainInfo.PactCollectStatus))
            {
                ShowErrorMessage(scanCode, "药品[" + collectMainInfo.DrugName + "]采集状态非法!");
                return;
            }

            if (TraceCodeCollectionStatusEnum.IsCollectCompleted(collectMainInfo.PactCollectStatus))
            {
                ShowErrorMessage(scanCode, "药品[" + collectMainInfo.DrugName + "]已经采集完成!");
                return;
            }
            ...
            //1.发药时是否已经采集过追溯码
            //2.采集的追溯码是否与发药时一致
            if (collectMainInfo.CollectType == CollectTypeEnum.ReturnOfGoods)
            {

            }

            var rowIndex = GetDrugCodeFPRowIndex(drugCode);
            if (rowIndex == -1)
            {
                ShowErrorMessage(scanCode, "药品[" + collectMainInfo.DrugName + "]未找到对应行索引!");
                return;
            }
            ...
            //if (collectMainInfo.IsHaveSplit == YesNoEnum.Yes)
            //{
            //    SetSplitTraceCodeFPColumnValue(rowIndex, collectMainInfo);
            //}
            ...
            YbTraceCollectMain info)
        {
            ...
            {
                ShowErrorMessage(scanCode, "药品[" + info.DrugName + "]" + TraceCodeCollectionStatusEnum.GetDescription(info.PactCollectStatus) + "!");
                return;
            }
...
: Math.Round((info.PactActualCollectQty / info.PactNeedCollectQty) * 100, 2).ToString("0.##") + "%";
    ...
    Convert.ToDecimal((info.CollectEndTime - info.CollectStartTime).TotalMilliseconds);
        ...
        private void btn_AutoAssign_Click(object sender, EventArgs e)
        {
            ...
            // 获取所有申请单号
            var applyNumberList = list
                .Where(x => !string.IsNullOrEmpty(x.ApplyNumber))
                .Select(x => x.ApplyNumber)
                .Distinct()
                .ToList();

            // 1. 查询当前发药申请明细
            var applyList = this.QueryService.GetApplyList(applyNumberList);
            if (applyList == null || applyList.Count == 0)
            {
                MessageBox.Show("未查询到申请数据！");
                return;
            }
            ...
            // 2. 遍历每条申请，查询原发药追溯码并赋值
            for (int i = 0; i < this.fpSpread1_Sheet1.RowCount; i++)
            {
                var collectInfo = this.fpSpread1_Sheet1.Rows[i].Tag as YbTraceCollectMain;
                if (collectInfo == null)
                {
                    continue;
                }
                ...
                // 获取包装和拆零需采集数量
                int pactRequired = (int)collectInfo.PactNeedCollectQty;
                int splitRequired = (int)collectInfo.SplitNeedCollectQty;

                // 都不需要采集的跳过
                if (pactRequired <= 0 && splitRequired <= 0)
                {
                    continue;
                }

                // 包装和拆零都已采集完成的跳过
                bool pactCompleted = pactRequired <= 0 || TraceCodeCollectionStatusEnum.IsCollectCompleted(collectInfo.PactCollectStatus);
                bool splitCompleted = splitRequired <= 0 || TraceCodeCollectionStatusEnum.IsCollectCompleted(collectInfo.SplitCollectStatus);
                if (pactCompleted && splitCompleted)
                {
                    continue;
                }

                // 找到对应的申请明细
                var apply = applyList.FirstOrDefault(x => x.ApplyNumber.ToString() == collectInfo.ApplyNumber);
                if (apply == null)
                {
                    failCount++;
                    failMessages.Add(collectInfo.DrugName + "：未找到申请明细");
                    continue;
                }

                // 3. 查询发药申请对应的费用明细信息(内含原医嘱流水号)
                var feeInfo = this.QueryService.GetFeeDetailInfo(
                    apply.PatientId,
                    apply.RecipeNo,
                    apply.SequenceNo.ToString(),
                    apply.MoOrder);

                if (feeInfo == null || string.IsNullOrEmpty(feeInfo.OldMoOrder))
                {
                    failCount++;
                    failMessages.Add(collectInfo.DrugName + "：未找到费用明细");
                    continue;
                }

                // 4. 根据feeInfo的old_mo_order等信息查询原发药申请流水号
                var origApplyNumber = this.QueryService.GetOriginalApplyNumber(
                    apply.PatientId,
                    feeInfo.OldMoOrder,
                    apply.RecipeNo,
                    apply.SequenceNo.ToString());

                if (string.IsNullOrEmpty(origApplyNumber))
                {
                    failCount++;
                    failMessages.Add(collectInfo.DrugName + "：未找到原发药申请");
                    continue;
                }

                // 5. 查询发药和退费采集的追溯码
                var traceMap = this.QueryService.GetTraceCollectInfoByApplyNumber(origApplyNumber);
                if (traceMap == null)
                {
                    failCount++;
                    failMessages.Add(collectInfo.DrugName + "：查询追溯码失败");
                    continue;
                }

                // 获取发药采集信息（销售 collect_type='0'）
                YbTraceCollectMain saleInfo = null;
                traceMap.TryGetValue("0", out saleInfo);

                // 获取退费采集信息（退货 collect_type='1'）
                YbTraceCollectMain returnInfo = null;
                traceMap.TryGetValue("1", out returnInfo);

                bool assignSuccess = true;

                // ========== 6. 处理包装追溯码 ==========
                if (!pactCompleted && pactRequired > 0)
                {
                    if (saleInfo == null || saleInfo.PactTracCodgsList == null || saleInfo.PactTracCodgsList.Count == 0)
                    {
                        failMessages.Add(collectInfo.DrugName + "（包装）：发药时未采集追溯码");
                        assignSuccess = false;
                    }
                    else
                    {
                        // 计算可用包装追溯码（发药的 - 退费的）
                        var usedPactCodes = new HashSet<string>();
                        if (returnInfo != null && returnInfo.PactTracCodgsList != null)
                        {
                            foreach (var code in returnInfo.PactTracCodgsList)
                            {
                                usedPactCodes.Add(code);
                            }
                        }

                        var availablePactCodes = saleInfo.PactTracCodgsList
                            .Where(c => !usedPactCodes.Contains(c))
                            .ToList();

                        // 校验可用数量
                        if (pactRequired != availablePactCodes.Count)
                        {
                            failMessages.Add(string.Format("{0}（包装）：可用({1})与需采集({2})不匹配",
                                collectInfo.DrugName, availablePactCodes.Count, pactRequired));
                            assignSuccess = false;
                        }
                        else
                        {
                            // 赋值包装追溯码
                            if (collectInfo.PactTracCodgsList == null)
                            {
                                collectInfo.PactTracCodgsList = new List<string>();
                            }

                            foreach (var code in availablePactCodes)
                            {
                                if (!collectInfo.PactTracCodgsList.Contains(code) && !AlreadyScannedTraceCodes.Contains(code))
                                {
                                    collectInfo.PactTracCodgsList.Add(code);
                                    AlreadyScannedTraceCodes.Add(code);
                                }
                            }

                            collectInfo.PactTracCodgs = string.Join(";", collectInfo.PactTracCodgsList.ToArray());
                            collectInfo.PactActualCollectQty = collectInfo.PactTracCodgsList.Count;
                            collectInfo.PactCollectStatus = TraceCodeCollectionStatusEnum.Sucess;
                        }
                    }
                }

                // ========== 7. 处理拆零追溯码 ==========
                if (!splitCompleted && splitRequired > 0)
                {
                    if (saleInfo == null || saleInfo.SplitTracCodgsList == null || saleInfo.SplitTracCodgsList.Count == 0)
                    {
                        failMessages.Add(collectInfo.DrugName + "（拆零）：发药时未采集追溯码");
                        assignSuccess = false;
                    }
                    else
                    {
                        // 计算可用拆零追溯码（发药的 - 退费的）
                        var usedSplitCodes = new HashSet<string>();
                        if (returnInfo != null && returnInfo.SplitTracCodgsList != null)
                        {
                            foreach (var code in returnInfo.SplitTracCodgsList)
                            {
                                usedSplitCodes.Add(code);
                            }
                        }

                        var availableSplitCodes = saleInfo.SplitTracCodgsList
                            .Where(c => !usedSplitCodes.Contains(c))
                            .ToList();

                        // 校验可用数量
                        if (splitRequired != availableSplitCodes.Count)
                        {
                            failMessages.Add(string.Format("{0}（拆零）：可用({1})与需采集({2})不匹配",
                                collectInfo.DrugName, availableSplitCodes.Count, splitRequired));
                            assignSuccess = false;
                        }
                        else
                        {
                            // 赋值拆零追溯码
                            if (collectInfo.SplitTracCodgsList == null)
                            {
                                collectInfo.SplitTracCodgsList = new List<string>();
                            }

                            foreach (var code in availableSplitCodes)
                            {
                                if (!collectInfo.SplitTracCodgsList.Contains(code))
                                {
                                    collectInfo.SplitTracCodgsList.Add(code);
                                }
                            }

                            collectInfo.SplitTracCodgs = string.Join(";", collectInfo.SplitTracCodgsList.ToArray());
                            collectInfo.SplitActualCollectQty = collectInfo.SplitTracCodgsList.Count;
                            collectInfo.SplitCollectStatus = TraceCodeCollectionStatusEnum.Sucess;
                        }
                    }
                }
                ...
                // 9. 刷新行显示
                SetCollectQtyFPColumnValue(i, collectInfo);
                SetCollectProgressFPColumnValue(i, collectInfo);
                SetPactTraceCodeFPColumnValue(i, collectInfo);
                SetSplitTraceCodeFPColumnValue(i, collectInfo);
                this.fpSpread1_Sheet1.Rows[i].Tag = collectInfo;
                AutoAdjustRowHeight(i);
            }
            ...
            {
                MessageBox.Show(string.Format("一键赋码完成，成功赋码 {0} 个药品！", successCount),
                    "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else if (successCount > 0)
            {
                string msg = string.Format("一键赋码完成，成功 {0} 个，失败 {1} 个\n\n失败原因：\n", successCount, failCount);
                msg += string.Join("\n", failMessages.Take(5).ToArray());
                if (failMessages.Count > 5)
                {
                    msg += string.Format("\n...还有 {0} 条", failMessages.Count - 5);
                }
                MessageBox.Show(msg, "部分成功", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if (failCount > 0)
            {
                string msg = string.Format("一键赋码失败，共 {0} 个药品\n\n失败原因：\n", failCount);
                msg += string.Join("\n", failMessages.Take(5).ToArray());
                if (failMessages.Count > 5)
                {
                    msg += string.Format("\n...还有 {0} 条", failMessages.Count - 5);
            ...
            {
                MessageBox.Show("没有需要赋码的药品", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
