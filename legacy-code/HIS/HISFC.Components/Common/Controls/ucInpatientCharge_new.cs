...
using System.Linq;
using HisCallExternalServiceProject.FunctionModule.SPDModule.Forms;
...
namespace Neusoft.HISFC.Components.Common.Controls
{
    ...
    public partial class ucInpatientCharge_new : UserControl, Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        ...
        /// 药品管理类
        /// </summary>
        protected Neusoft.HISFC.BizLogic.Pharmacy.Item phaItemMgr = new Neusoft.HISFC.BizLogic.Pharmacy.Item();
        ...
        /// 医嘱扩展信息管理
        /// </summary>
        Neusoft.HISFC.BizLogic.Order.OrderExtend orderExtMgr = new Neusoft.HISFC.BizLogic.Order.OrderExtend();
        ...
        /// 是否弹出发票
        /// </summary>
        [Category("控件设置"), Description("是否显示发票分类弹框")]
        ...
        protected virtual int AddChargeDetail(Neusoft.HISFC.Models.Base.Item item, int row, string execDeptCode, bool isFeegroup)
        {
            ...
            if (item.ItemType != EnumItemType.MatItem)
            {
                ...
                if (this.undrugManager.SetUltrasound(item.ID))//查询项目是否为四肢血管项目
                {
                    ...
                    if (counts > 1)
                    {
                        ...
                        price = price - pricece;//开立多次减去价格差额
                    }
                }
                item.Price = price;
            ...
            //if (item.ItemType == EnumItemType.Drug)//药品
            //{
            //    if (hsLimit.Contains(item.UserCode))
            //    {
            //        Neusoft.HISFC.Models.Base.Const dic = new Const();
            //        dic = hsLimit[item.UserCode] as Neusoft.HISFC.Models.Base.Const;
            //        Forms.frmLimitation limFrm = new Neusoft.HISFC.Components.Common.Forms.frmLimitation();
            //        limFrm.Memo = dic.Memo;//hsLimit[limitTemp.UserCode].ToString();
            //        limFrm.drugName = item.Name;
            //        limFrm.ShowDialog();
            //        string isLimit = limFrm.IsLimit;
            //        limFrm.Dispose();
            //        Neusoft.HISFC.Models.Order.Inpatient.OrderExtend ordExt = new Neusoft.HISFC.Models.Order.Inpatient.OrderExtend();
            //        ordExt.InPatientNo = this.patientInfo.ID;
            //        //限制药标识
            //        ordExt.Extend3 = isLimit;
            //        feeitemlist.Order.OrdExtend = ordExt.Clone();
        ...
        public virtual bool IsValid()
        {
            ...
            for (int i = 0; i < this.fpDetail_Sheet.RowCount; i++)
            {
                ...
                if (this.isUseAdmin)
                {
                    ...
                    if (item.User01 == "1" && string.IsNullOrEmpty(adminCode))
                    {
        ...
        protected virtual int Fee()
        {
            ...
            //var re = CheckWJJK(this.patientInfo.ID);
            //if (re < 0)
            //{
            ...
            for (int i = 0; i < this.fpDetail_Sheet.RowCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(feeItemList.Item.SpdUniqueCode))
                {
                    ...
                    {
                        MessageBox.Show("SPD高值耗材只允许收费数量为1，当前【" + feeItemList.Item.SpdUniqueCode + "】收费数量为" + feeItemList.Item.Qty);
                        return -1;
                    }
                ...
                //获取进行判断是否为病理项目
                if (this.inpatientManager.QueryPathologyRemind(feeItemList.Item.GBCode) > 0 && Reminder == "0")
                {
                    if (this.inpatientManager.QueryPathologyOrder(feeItemList.ExtFlag2) == 0)
                    {
                        Reminder = "1";//满足条件后进行标记
                        MessageBox.Show("该患者未开具病理医嘱，请联系医生补录病理医嘱。");
                ...
                else
                {
                    ...
                    if (feeItemList.Item.ItemType == EnumItemType.Drug)
                    {
                        if (this.pharmacyIntergrate.ApplyOut((Neusoft.HISFC.Models.RADT.PatientInfo)this.fpDetail_Sheet.Cells[i, (int)Columns.InpatientNO].Tag, feeItemList, operTime, false) == -1)
                        {
                            feeIntergrate.Rollback();
            ...
            for (int ii4 = 0; ii4 < AlFirstInputFeeItemlist.Count; ii4++)
            {
                ...
                this.ApplyPricingPreviewToFeeDetails(this.patientInfo, firstInputFeeItemlistTemp, operTime, employee, employee.Dept == null ? string.Empty : employee.Dept.ID);
                ...
                foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList feeItemList in firstInputFeeItemlistTemp)
                {
                    ...
                    if (feeItemList.Item.ItemType == EnumItemType.Drug)
                    {
                        ...
                        //限制药插入医嘱扩展表 20190724
                        //if (!string.IsNullOrEmpty(feeItemList.Order.OrdExtend.Extend3))
                        //{
                        //    feeItemList.Order.OrdExtend.MoOrder = feeItemList.Order.ID;
                        //    feeItemList.Order.OrdExtend.InPatientNo = this.patientInfo.ID;
                        //    if (orderExtMgr.InsertOrderExtend(feeItemList.Order.OrdExtend) == -1)
                        //    {
                        //        feeIntergrate.Rollback();
            ...
            if (this.IsPopSHowInvoice)
            {
                ...
                Hashtable allFeeByInvoice = this.GetFeeByInvoice(alFeeInfo, ref allInvoiceFeeType);
        ...
        private int SplitUndrugCombItem(ref ArrayList itemList)
        {
            ...
            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList f in itemList)
            {
                ...
                else//单条的新加的给加上ID add by yerl
                {
                    ...
                    //重新计算价格优惠比例 20191201 by zyq
                    //rate = feeIntergrate.GetItemRateForZT(f.Item.ID, f.ID);

                    ////拆分时 重新取收费价
                    //if (this.feeIntergrate.GetPriceForInpatient(this.patientInfo, f.Item, ref price, ref orgPrice, rate) < 0)
                    //{
                    //    MessageBox.Show(Language.Msg("获取组套明细的价格失败!") + this.feeIntergrate.Err);
                    //    return -1;
                    //}
            ...
            foreach (Neusoft.HISFC.Models.Fee.Inpatient.FeeItemList f in undrugCombItemList)
            {
                ...
                for (int i = 0; i < details.Count; i++)
                {
                    ...
                    /*旧代码，这里没有计算优惠比例，导致价格出错,注释掉 这坑估计从上线开始就有了  20191201 by zyq
                    ////拆分时 重新取收费价
        ...
        private PatientInfo GetPricingPreviewPatient(int row, FeeItemList fee)
        {
            object patientObject = this.fpDetail_Sheet.Cells[row, (int)Columns.InpatientNO].Tag;
            if (patientObject is PatientInfo)
            {
                return patientObject as PatientInfo;
            }

            if (fee != null && !string.IsNullOrEmpty(fee.ExtFlag2))
            {
                return this.radtIntegrate.GetPatientInfomation(fee.ExtFlag2);
            }

            return this.patientInfo;
        }

        private ArrayList CollectPricingPreviewFeeDetails(ArrayList rowIndexes, ArrayList patients, DateTime operTime, Employee employee)
        {
            ArrayList feeDetails = new ArrayList();
            if (rowIndexes == null || patients == null)
            {
                return feeDetails;
            }
            ...
            for (int row = 0; row < this.fpDetail_Sheet.RowCount; row++)
            {
                ...
                PatientInfo patient = this.GetPricingPreviewPatient(row, fee);
        ...
        {
            if (this.isPricingPreviewRefreshing)
            {
                return;
            }
            ...
            ArrayList feeDetails = this.CollectPricingPreviewFeeDetails(rowIndexes, patients, operTime, employee);
            ...
            Hashtable groupFees = new Hashtable();
            Hashtable groupPatients = new Hashtable();
            for (int i = 0; i < feeDetails.Count; i++)
            {
                PatientInfo patient = patients[i] as PatientInfo;
                FeeItemList fee = feeDetails[i] as FeeItemList;
                string key = patient == null ? string.Empty : patient.ID;
                if (string.IsNullOrEmpty(key) && fee != null)
                {
                    key = fee.ExtFlag2;
                }
                ...
                {
                    this.ApplyPricingPreviewFailureMemo((int)rowIndexes[i], Neusoft.HISFC.BizProcess.Integrate.PricingInpatientPreviewResult.Failed("统一计价试算缺少住院患者信息。"));
                    continue;
                }

                if (!groupFees.ContainsKey(key))
                {
                    groupKeys.Add(key);
                    groupRows[key] = new ArrayList();
                    groupFees[key] = new ArrayList();
                    groupPatients[key] = patient;
                }

                ((ArrayList)groupRows[key]).Add(rowIndexes[i]);
                ((ArrayList)groupFees[key]).Add(fee);
            }
            ...
            try
            {
                ...
                {
                    ArrayList rows = groupRows[key] as ArrayList;
                    ArrayList fees = groupFees[key] as ArrayList;
                    PatientInfo patient = groupPatients[key] as PatientInfo;
                    Neusoft.HISFC.BizProcess.Integrate.PricingInpatientPreviewResult previewResult =
                        this.GetPricingPreview(patient, fees, operTime, employee, fallbackDeptCode);
                    if (previewResult != null
                        && previewResult.Configured
                        && previewResult.Success
                        && previewResult.Items.Count != fees.Count)
                    {
        ...
        private List<Const> SDPPackageItemList = new List<Const>();

        private bool IsSPDScanPackageItem(string itemCode)
        {
            if (!SDPPackageItemList.Any())
            {
                SDPPackageItemList = conMgr.GetConstantList("SDP_PackageItem").Cast<Const>().ToList();
            }

            return SDPPackageItemList.Any(a => a.ID == itemCode);

        }
        /// <summary>
        /// 当收费特殊套包时，弹出扫码框
        /// </summary>
        private void ShowPackageScanForm(Item item, int currRow)
        {
            try
            {
                var SPDScanPackageItemResult = IsSPDScanPackageItem(item.ID);
                if (!SPDScanPackageItemResult)
                {
                    return;
                }
                ...
                //弹出扫码框
                frmScanSPDCode f = new frmScanSPDCode();
                f.SetTitle("SPD套包扫码--" + item.Name);
                f.ShowDialog();

                var result = f.DialogResult;
                if (result != DialogResult.Yes)
                {
                    return;
                }
                foreach (var spdInfo in f.ScanSpdPackageList)
                {
                    spdInfo.InPatientNo = this.patientInfo.ID;
                    spdInfo.FeetemCode = item.ID;
                    spdInfo.FeeItemName = item.Name;
                }

                item.ScanSpdPackageList = f.ScanSpdPackageList;

                this.fpDetail_Sheet.Rows[currRow].Tag = item;

                //MessageBox.Show("共扫码成功：" + item.ScanSpdPackageList.Count + "条");

            }
            catch (Exception ex)
            {
                MessageBox.Show("扫码期间出现异常:" + ex.Message);
            }
        ...
        private int SetItem(int row, PayTypes payType, string recipeDeptCode, DateTime operTime, ref bool isNewItem, ref FeeItemList feeItemList)
        {
            ...
            //SPD套包收费 rfid赋值
            var item = this.fpDetail_Sheet.Rows[row].Tag as Item;
            if (item.ScanSpdPackageList != null && item.ScanSpdPackageList.Any())
            {
                feeItemList.ScanSpdPackageList = item.ScanSpdPackageList;
            }
        ...
        private int RefreshGroupList()
        {
            ...
            this.AddDeptGroup(root, (Neusoft.FrameWork.Management.Connection.Operator as Neusoft.HISFC.Models.Base.Employee).Dept.ID);
            root.Expand();
        ...
        /// 扫码高值耗材 
        /// </summary>
        public void ScanCodeHighHC()
        {

            HISFC.Components.Common.Forms.frmScanInput frmScan = new Neusoft.HISFC.Components.Common.Forms.frmScanInput();
            string itemCode = "";
            frmScan.Text = "请扫码高值耗材码";
            if (frmScan.ShowDialog() == DialogResult.OK)
            {
                string strQRCode = frmScan.QRCode;
                if (!spdService.SPDSwitch)
                {
                ...
                this.SetHighHC(strQRCode, itemCode);
            }
        }

        public Dictionary<string, string> SPDCodeRelHisCodeDic = new Dictionary<string, string>();

        public int SetHighHC(string spdUniqueCode, string itemCode)
        {
            try
            {
        ...
        public virtual int Init(string deptCode)
        {
            ...
            //珠海限制用药 20190724
            //ArrayList alLimit = conLogic.GetAllList("ZHUHAILIMITDRUG");
        ...
        private void tvGroup_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            Neusoft.HISFC.Models.Fee.ComGroup comGroup = null;
            try
            {
