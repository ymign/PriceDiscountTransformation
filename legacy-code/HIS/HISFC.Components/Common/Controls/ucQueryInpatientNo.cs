...
namespace Neusoft.HISFC.Components.Common.Controls
{
    ...
    public partial class ucQueryInpatientNo : UserControl
    {
        ...
        private void SelectPatient()
        {
            ...
            for (int i = 0; i < this.alInpatientNos.Count; i++)
            {
                ...
                if (b)
                {
                    ...
                    try
                    {
                        if (obj.GetType() == typeof(Neusoft.HISFC.Models.Base.Spell))
                        {
                            Neusoft.HISFC.Models.Base.Spell spTemp = (Neusoft.HISFC.Models.Base.Spell)obj;
        ...
        private ArrayList Filter(ArrayList alPatient)
        {
            ...
            {
                //按照科室过滤
                foreach (Neusoft.FrameWork.Models.NeuObject obj in alPatient)
                {
                    if (this.isDeptOnly)
                    {
                        if (obj.GetType() == typeof(Neusoft.HISFC.Models.Base.Spell))
                        {
                            Neusoft.HISFC.Models.Base.Spell sp = obj as Neusoft.HISFC.Models.Base.Spell;
                            //obj.User01存在院科室信息
                            if (sp.User01.Trim() == ((Neusoft.HISFC.Models.Base.Employee)Neusoft.FrameWork.Management.Connection.Operator).Dept.ID ||
                                sp.UserCode.Trim() == ((Neusoft.HISFC.Models.Base.Employee)Neusoft.FrameWork.Management.Connection.Operator).Dept.ID)
                            {
                                alTemp.Add(obj);
                            }
                        }
                        ...
                        {
                            //obj.User01存在院科室信息
                            if (obj.User01.Trim() == ((Neusoft.HISFC.Models.Base.Employee)Neusoft.FrameWork.Management.Connection.Operator).Dept.ID)
                            {
                                alTemp.Add(obj);
                            }
                        }
        ...
        {
            if (this.ParentForm is Neusoft.HISFC.Components.Common.Forms.frmPatientView)
            {
                return;
            }
            Neusoft.HISFC.Components.Common.Forms.frmPatientView frmPob = new Neusoft.HISFC.Components.Common.Forms.frmPatientView();

            frmPob.SelectedPatientinfo += new Neusoft.HISFC.Components.Common.Forms.frmPatientView.SelectPatientInfoDelagate(frmPob_SelectedPatientinfo);
            frmPob.ShowDialog();
        ...
        {
            this.txtInputCode.Text = (sender as Neusoft.HISFC.Models.RADT.PatientInfo).PID.PatientNO;
            this.strInpatientNo = (sender as Neusoft.HISFC.Models.RADT.PatientInfo).ID;
        }
    }
