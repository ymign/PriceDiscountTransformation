...
namespace Neusoft.HISFC.BizLogic.Privilege
{
    ...
    public class SecurityService : DataBase
    {
        ...
        /// <returns>1 为可以登录 </returns>
        public int LimitLogin(string userId, ref string msg)
        {
            string use = "0";//是否启用
            int PWErroTimes = 0;//密码错误次数
            int ULLimitTimes = 0;//密码错误时间段
            int ULLockTimes = 0;//用户锁定时间

            UserLogic userlogin = new UserLogic();

            userlogin.GetLimitLoginSetting(ref use, ref PWErroTimes, ref ULLockTimes, ref ULLimitTimes);
            ...
            Privilege.Model.LimitUserLogin loginobj = new LimitUserLogin();
            loginobj = userlogin.QueryLimitUserLogin(userId);
            if (loginobj == null || string.IsNullOrEmpty(loginobj.Empl_code))
            {
                return 1;
            }
            DateTime curdatetime = this.GetDateTimeFromSysDateTime();
            DateTime religintime = Neusoft.FrameWork.Function.NConvert.ToDateTime(loginobj.Relogin_time);
            if (curdatetime < religintime && loginobj.Lockflag == "1")
            {
                msg = "密码错误" + loginobj.Times + "次,请在" + religintime.ToString() + "后,再次登录!";
                return -1;
            }
            loginobj.Times = 1;
            loginobj.Lockflag = "0";
            userlogin.UpdateLimitLoginInfo(loginobj);
            return 1;
        }
        ...
        /// <returns></returns>
        public int UpdateLimitLogin(string userId, ref string msg)
        {
            string use = "0";//是否启用
            int PWErroTimes = 0;//密码错误次数
            int ULLimitTimes = 0;//密码错误时间段
            ...
            if (loginobj == null || string.IsNullOrEmpty(loginobj.Empl_code))
            {
                loginobj = new LimitUserLogin();
                loginobj.Empl_code = userId;
                loginobj.Name = "";
                loginobj.Login_time = curdatetime.ToString();
                loginobj.Limitlogin_time = curdatetime.AddMinutes(ULLimitTimes).ToString();//添加限制时间
                loginobj.Relogin_time = curdatetime.AddMinutes(ULLimitTimes).ToString();//添加重新登录时间限制
                loginobj.Times = 1;
                loginobj.Lockflag = "0";
            }
            else
            {
                DateTime limittime = Neusoft.FrameWork.Function.NConvert.ToDateTime(loginobj.Limitlogin_time);
                if (curdatetime <= limittime)
                {
                    loginobj.Times++;
                    if (loginobj.Times >= PWErroTimes)//密码错误次数
                    {
                        loginobj.Lockflag = "1";
                    }
                    loginobj.Relogin_time = this.GetDateTimeFromSysDateTime().AddMinutes(ULLimitTimes).ToString();//添加重新登录时间限制
                }
                else
                {
                    loginobj.Login_time = curdatetime.ToString();
            ...
            if (userlogin.UpdateLimitLoginInfo(loginobj) <= 0)
            {
                if (userlogin.InsertLimitLoginInfo(loginobj) <= 0)
                {
                    return -1;
                }
            }
