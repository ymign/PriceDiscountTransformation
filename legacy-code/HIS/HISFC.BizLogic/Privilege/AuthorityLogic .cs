...
namespace Neusoft.HISFC.BizLogic.Privilege
{
    ...
    public class AuthorityLogic : DataBase
    {
        ...
        public List<String[]> Query(string userId)
        {
            ...
            try
            {
                sql = string.Format(sql, userId,Neusoft.FrameWork.Management.Connection.Hospital.ID);
            }
        ...
        public int Delete(string userId, string roleId)
        {
            ...
            try
            {
                sql = string.Format(sql,userId,roleId,Neusoft.FrameWork.Management.Connection.Hospital.ID);
            }
            catch (Exception ex) { this.Err = ex.Message; return -1; }
            if (this.ExecNoQuery(sql) <= 0) return -1;
            return 0;
        ...
        public int Delete(string Id)
        {
            ...
            try
            {
                sql = string.Format(sql, Id,Neusoft.FrameWork.Management.Connection.Hospital.ID);
            }
            catch (Exception ex) { this.Err = ex.Message; return -1; }
            if (this.ExecNoQuery(sql) <= 0) return -1;
            return 0;
