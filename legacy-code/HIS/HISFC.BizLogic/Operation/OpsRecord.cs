...
namespace Neusoft.HISFC.BizLogic.Operation
{
	...
	public abstract class OpsRecord : Neusoft.FrameWork.Management.Database
	{
        ...
        private ArrayList QueryMyOperatorRecord(string strSql)
        {
            ...
            try
            {
                while (this.Reader.Read())
                {
                    ...
                    {
                        Neusoft.HISFC.Models.Registration.Register regObj = this.GetRegInfo(record.OperationAppllication.PatientInfo.ID);
                    ...
                    record.IsRescue = Neusoft.FrameWork.Function.NConvert.ToBoolean(Reader[70]);
                    //{5DFF5830-8094-4ee0-A830-93731510284C}
                    record.IsOtherHosDoc = Neusoft.FrameWork.Function.NConvert.ToBoolean(Reader[71]);
                    record.SpecialDevoice = Reader[72].ToString();
                    //{455F0D5D-89B7-4e06-974A-931534B52AC3}手术结束时间
                    record.OpsEndDate = Neusoft.FrameWork.Function.NConvert.ToDateTime(Reader[73].ToString());

                    #region 抗生素使用

                    record.IsUseAntibio = Neusoft.FrameWork.Function.NConvert.ToBoolean(Reader[74].ToString());
                    record.UseAntibio.ID = Reader[75].ToString();
                    record.UseAntibio.Name = Reader[76].ToString();
                    record.UseAntDosage = Neusoft.FrameWork.Function.NConvert.ToDecimal(Reader[77].ToString());
                    record.IsUseSecondAnt = Neusoft.FrameWork.Function.NConvert.ToBoolean(Reader[78].ToString());
                    record.UseAntTime = Neusoft.FrameWork.Function.NConvert.ToDateTime(Reader[79].ToString());
		...
		public int AddOperatorRecord(Neusoft.HISFC.Models.Operation.OperationRecord OpsRecord)
		{
            ...
            //{5DFF5830-8094-4ee0-A830-93731510284C}
            string intIsOtherHosDoc = Neusoft.FrameWork.Function.NConvert.ToInt32(OpsRecord.IsOtherHosDoc).ToString();
			...
			if(this.Sql.GetSql("Operator.OpsRecord.AddOperatorRecord.2",ref strSql)==-1) 
			{
				return -1;
			}
			...
			{				
				...
				string []str = new string[]{
                                               ...
                                               OpsRecord.IsUseAntibio ? "1" : "0" , //术前预防是否使用抗生素 0-否 1-是
                                                OpsRecord.UseAntibio.ID, //抗生素编码
                                                OpsRecord.UseAntibio.Name, //抗生素名称
                                                OpsRecord.UseAntDosage.ToString(), //抗生素剂量
                                                OpsRecord.IsUseSecondAnt ? "1" : "0", //术中是否追加第二剂抗菌药物  0-否 1-是
                                                OpsRecord.UseAntTime.ToString("yyyy-MM-dd HH:mm:ss") //抗生素使用时间
										   };
...
//					OpsRecord.m_objOpsApp.PatientInfo.Weight,this.Operation.ID.ToString(),OpsRecord.m_objOpsApp.RoomID,strOperDate,OpsRecord.ForePress);
		...
		{				
			...
			string strFee = Neusoft.FrameWork.Function.NConvert.ToInt32(OpsRecord.IsCharged).ToString();
            //{5DFF5830-8094-4ee0-A830-93731510284C}
            string strIsOtherHosDoc = Neusoft.FrameWork.Function.NConvert.ToInt32(OpsRecord.IsOtherHosDoc).ToString();
			if(OpsRecord.OperationAppllication.PatientInfo.Weight == "") 
				OpsRecord.OperationAppllication.PatientInfo.Weight = "0";
			//by zlw
	        string docDept = OpsRecord.OperationAppllication.OperationDoctor.Dept.ID;
			if(this.Sql.GetSql("Operator.OpsRecord.UpdateOperatorRecord.2",ref strSql)==-1) return -1;
			try
			{				
				//手术登记表中增加记录
				//每行5个参数
				string []str = new string[]{
                                               ...
                                               OpsRecord.IsUseAntibio ? "1" : "0" , //术前预防是否使用抗生素 0-否 1-是
                                                ...
                                                OpsRecord.UseAntDosage.ToString(), //抗生素剂量
                                                OpsRecord.IsUseSecondAnt ? "1" : "0", //术中是否追加第二剂抗菌药物  0-否 1-是
                                                OpsRecord.UseAntTime.ToString("yyyy-MM-dd HH:mm:ss") 
										   };
//				strSql = string.Format(strSql,OpsApp.OperationNo,ls_ClinicCode,ls_PatientNo,OpsApp.Pasource,ls_Name,
//					ls_DeptCode,ls_BedNo,ls_Sex,ldt_Birthday.ToString(),ls_BloodCode,
//					OpsApp.OperateKind,OpsApp.Ops_docd.ID.ToString(),OpsApp.Gui_docd.ID.ToString(),OpsApp.Pre_Date.ToString(),OpsApp.Duration.ToString(),
...
//			if(this.ExecNoQuery(strSql) == -1) return -1;
		...
		{
            Neusoft.FrameWork.Management.ControlParam ctrlLogicInstance = new Neusoft.FrameWork.Management.ControlParam();
