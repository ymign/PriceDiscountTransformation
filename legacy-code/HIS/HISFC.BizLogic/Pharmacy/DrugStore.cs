...
namespace Neusoft.HISFC.BizLogic.Pharmacy
{
    ...
    public class DrugStore : DataBase
    {
        ...
        /// <param name="strSQL">查询的SQl语句</param>
        /// <returns>成功返回数组 失败返回null</returns>
        public ArrayList myGetDrugRecipeInfo(string strSQL)
        {
            ArrayList al = new ArrayList();
            if (this.ExecQuery(strSQL) == -1)
            {
        ...
        /// <param name="strSQL">查询的SQl语句</param>
        /// <returns>成功返回数组 失败返回null</returns>
        public ArrayList GetRecipeBySql(string strSQL)
        {
            ArrayList al = new ArrayList();
            if (this.ExecQuery(strSQL) == -1)
            {
        ...
        public int UpdateDrugRecipeValidState(string recipeNo, string class3MenaingCode, Neusoft.HISFC.Models.Base.EnumValidState validState)
        {
            ...
            Neusoft.FrameWork.WinForms.Classes.HisLog.WriteLog("QuitFee", strSql);
            Neusoft.FrameWork.WinForms.Classes.HisLog.WriteLog("QuitFee", "result:" + param.ToString());
            return param;
        }
        ...
        /// <param name="drugedNum">本次已配药数量</param>
        /// <returns>成功返回1 失败返回－1</returns>
        public int UpdateDrugRecipeDrugedInfoZYF(string drugDept, string recipeNo, string class3MeaningCode, string drugOper, string drugedDept, decimal drugedNum)
        {

            string strSql = "";
            if (this.GetSQL("Pharmacy.DrugStore.UpdateDrugRecipeInfo.DrugedZYF", ref strSql) == -1)
                return -1;
            try
            {
                string[] strParm = {	
        ...
        /// <param name="drugSendInfo">处方调剂信息 发药药房+发药窗口</param>        
        /// <returns>成功返回1 失败返回-1 </returns>
        public int DrugRecipeZYF(Neusoft.HISFC.Models.Registration.Register patient, ArrayList feeAl, string feeWindow, out string drugSendInfo)
        {
            return DrugRecipeZYF(patient, feeAl, feeWindow, null, out drugSendInfo);
        }
        ...
        public int DrugRecipe(Neusoft.HISFC.Models.Registration.Register patient, ArrayList feeAl, string feeWindow, System.Collections.Hashtable hsDeptAddress, out string drugSendInfo)
        {
            ...
            foreach (ArrayList temp in feeTempAl)
            {
                ...
                for (int i = 0; i < temp.Count; i++)
                {
                    ...
                    if (recipeNo != "" && recipeNo != feeInfo.RecipeNO)
                    {
                        if (alTemp.Count > 0)
                        {
                            ...
                            if (info.StockDept.ID == "9092")
                            {
                                info.RecipeState = "2";	//这是康美煎送药品的特殊处理，因为上线紧急，处理不当
                                info.DrugedOper.ID = "000000";
                                info.DrugedOper.OperTime = info.FeeOper.OperTime;
                            }
                ...
                if (alTemp != null && alTemp.Count > 0)
                {
                    ...
                    #region 这是康美煎送药品的特殊处理，因为上线紧急，处理不当
                    if (info.StockDept.ID == "9092")
                    {
                        info.RecipeState = "2";
                        info.DrugedOper.ID = "000000";
        ...
        /// <param name="drugSendInfo">处方调剂信息 发药药房+发药窗口</param>        
        /// <returns>成功返回1 失败返回-1 </returns>
        public int DrugRecipeZYF(Neusoft.HISFC.Models.Registration.Register patient, ArrayList feeAl, string feeWindow, System.Collections.Hashtable hsDeptAddress, out string drugSendInfo)
        {
            if (hsDeptAddress == null)
            {
                hsDeptAddress = new Hashtable();
        ...
        public ArrayList QueryDrugAuditing(string queryType, string states, string beginTime, string endTime)
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    Neusoft.HISFC.Models.Pharmacy.ApplyOut appObj = new ApplyOut();
        ...
        /// <returns></returns>
        public int InsertDrugAuditing(string queryType, Neusoft.FrameWork.Models.NeuObject tempObj)
        {
            string strSql = "";
        ...
        public ArrayList QueryDrugMessageInpatientList(Neusoft.HISFC.Models.Pharmacy.DrugControl drugControl)
        {
            string strSQL = "";
        ...
        /// <summary>
        /// 出院结算时更新出院带药状态，用于上住院药房大屏流程用
        /// </summary>
        /// <param name="patientId"></param>
        /// <returns></returns>
        public int UpdatePkState(string patientId)
        {
            try
            {
                string sql = @"update  pha_com_applyout a
set a.pkstatus='3'
where a.class3_meaning_code='Z1'
and a.billclass_code in('OP1','O')
and a.patient_id='{0}'
--and a.patient_id in (select f.inpatient_no from fin_ipr_inmaininfo f where f.in_state in('R','I','B'))
and a.valid_state='1'
and a.drug_dept_code='9008'
and a.apply_state in ('0','1')";

                sql = string.Format(sql, patientId);
                if (this.ExecNoQuery(sql) < 0)
                    return -1;

                string sql2 = @"insert into pha_soc_callqueue
  select a.recipe_no,
         a.patient_id,
         f.card_no,
         f.name,
    ...
    join fin_ipr_inmaininfo f on a.patient_id = f.inpatient_no
   where a.class3_meaning_code = 'Z1'
     and a.billclass_code in ('OP1', 'O')
     and a.patient_id = '{0}'
        --and a.patient_id in (select f.inpatient_no from fin_ipr_inmaininfo f where f.in_state in('R','I','B'))
     ...
     and a.apply_state in ('0', '1');
 ";
                sql2 = string.Format(sql2, patientId);
                return this.ExecNoQuery(sql2);
            }
            catch (Exception ex)
            {
                return -1;
        ...
        /// <returns></returns>
        public int InpatientDrugSpecial(ArrayList alApplyOut, Neusoft.HISFC.Models.Pharmacy.DrugMessage drugMessage)
        {
            ...
            string sql = @"update pha_com_applyout a
set a.pkstatus='4',a.druged_bill='{4}'
where a.apply_number='{0}'
and a.recipe_no='{1}'
and a.drug_dept_code='{2}'
and a.apply_state='{3}'
and a.valid_state='1'
";
            ...
            foreach (Neusoft.HISFC.Models.Pharmacy.ApplyOut item in alApplyOut)
            {
                string execStr = string.Format(sql, item.ID, item.RecipeNO, item.StockDept.ID, "0", drugMessage.DrugBillClass.Memo);
                if (this.ExecNoQuery(execStr) == -1)
                {
        ...
        /// <param name="alApplyOut"></param>
        /// <returns></returns>
        public int InpatientDrugSpecial2(ArrayList alApplyOut, Neusoft.HISFC.Models.Pharmacy.DrugMessage drugMessage)
        {
            if (alApplyOut.Count <= 0)
            {
                return -1;
...
";
        ...
        // {5B610F39-88F0-4f96-B6A6-B94B0FF976C4}  出院带药发药时扣库存处理,将pkstatus状态改为5
        /// <summary>
        /// 配药（出院带药）
        /// </summary>
        /// <param name="alApplyOut"></param>
        /// <returns></returns>
        public int InpatientDrugSpecial3(ArrayList alApplyOut, Neusoft.HISFC.Models.Pharmacy.DrugMessage drugMessage)
        {
            if (alApplyOut.Count <= 0)
            {
                return -1;
...
";
            ...
            foreach (Neusoft.HISFC.Models.Pharmacy.ApplyOut item in alApplyOut)
            {
                string execStr = string.Format(sql, item.ID);
                if (this.ExecNoQuery(execStr) == -1)
                {
        ...
        /// </summary>
        /// <returns></returns>
        public int InpatientCallQueue(Neusoft.HISFC.Models.Pharmacy.ApplyOut item, Neusoft.HISFC.Models.RADT.PatientInfo patient, Neusoft.HISFC.Models.Pharmacy.DrugControl DrugCtl)
        {
            #region sql

            string sql = @"insert into pha_soc_callqueue p
                        (P.RECIPE_NO,--1处方号
                        P.PATIENT_ID,--2门诊号
                        P.CARD_NO,--3病历号
                        P.PATIENT_NAME,--4患者姓名
                        P.PATIENT_SEX,--5患者性别
                        P.DEPT_CODE,--6看诊科室
                        P.DRUG_DEPT_CODE,--7取药科室
                        P.SEND_TERMINAL_CODE,--8发药终端编码
                        P.SEND_TERMINAL_NAME,--9发药终端名称
                        P.MEMO,--10备注
                        P.STATE,--11状态，0：待叫号 1已叫号 2已发药3延迟叫号
                        P.OPER_CODE,--12操作员工号
                        P.OPER_DATE--13操作时间
                        ) values
                        ('{0}',--1处方号
        ...
        public int UpdateApplyOutOper(string drugBillNo, string operCode)
        {
            string sql = "update pha_Com_applyout a set a.oper_code='{0}' where a.druged_bill='{1}'";

            try
            {
                sql = string.Format(sql, operCode, drugBillNo);
                return this.ExecNoQuery(sql);
            }
            catch (Exception ex)
            {
