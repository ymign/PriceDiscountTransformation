...
namespace Neusoft.HISFC.BizLogic.Fee
{
    ...
    public class ReturnApply : Neusoft.FrameWork.Management.Database
    {
        ...
        private ArrayList QueryReturnApplysBySql(string sql, params string[] args)
        {
            ...
            try
            {
                ...
                while (this.Reader.Read())
                {
                    ...
                    {
                        returnApply.UndrugComb.Qty = Neusoft.FrameWork.Function.NConvert.ToDecimal(this.Reader[37].ToString());
                    }
        ...
        /// </summary>
        /// <returns>成功:获得退费申请SELECT语句 失败:null</returns>
        private string GetReturnApplySelectSqlNew()
        {
            string sql = string.Empty;

            //取SQL语句
            if (this.Sql.GetCommonSql("Fee.ApplyReturn.GetApplyReturnListNew", ref sql) == -1)
            {
                this.Err = "没有找到索引为:Fee.ApplyReturn.GetApplyReturnList的SQL语句!";
        ...
        /// <param name="args">参数</param>
        /// <returns>成功: 退费申请信息 失败: null 没有找到数据ArrayList.Count = 0</returns>
        private ArrayList QueryReturnApplysNew(string whereIndex, params string[] args)
        {
            string select = string.Empty;
            string where = string.Empty;

            select = this.GetReturnApplySelectSqlNew();
        ...
        /// <param name="isCharged">是否被确认的数据</param>
        /// <returns>成功:退费申请数组 失败: null 没有查找到数据返回元素数为0的ArrayList</returns>
        public ArrayList QueryReturnApplysNew(string inpatientNO, bool isCharged)
        {
            return this.QueryReturnApplysNew("Fee.ApplyReturn.GetApplyReturnList.Where", inpatientNO, NConvert.ToInt32(isCharged).ToString());
        }
        /// <summary>
        /// 根据患者住院流水号，取此患者的退费申请数据
        /// </summary>
