...
namespace Neusoft.HISFC.BizLogic.HealthRecord.Visit
{
    ...
    public class LinkWay :Neusoft.FrameWork.Management.Database
    {
        ...
        /// <param name="linkWayObj">联系方式实体类</param>
        /// <returns>失败返回-1</returns>
        public int UpdateLinkWayPhone(Neusoft.HISFC.Models.HealthRecord.Visit.LinkWay linkWayObj)
        {
            //获取主sql语句
            string strSQL = string.Empty;
            if (this.Sql.GetSql("HealthReacord.Visit.LinkWay.UpdateLinkWay01", ref strSQL) == -1)
            {
                this.Err = "获取SQL语句:HealthReacord.Visit.LinkWay.UpdateLinkWay 失败";
                return -1;
            }

            strSQL = string.Format(strSQL, linkWayObj.Phone, linkWayObj.ID);
