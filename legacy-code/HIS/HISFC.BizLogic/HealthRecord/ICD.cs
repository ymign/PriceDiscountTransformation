...
namespace Neusoft.HISFC.BizLogic.HealthRecord
{
    ...
    public class ICD : Neusoft.FrameWork.Management.Database
    {
        ...
        // {82092764-007C-4402-8867-9EE620336A8D}  诊断增加一行空值，用于回车跳过
        /// <summary>
        ///  获得相应查询类别的ICD信息  用的时候 首先要判断返回值是
        ///   如果是0 则取DataSet 如果是-1 说明有错误  应该提示错误信息
        /// </summary>
        ...
        /// <returns>出现未知错误 返回 -1 成功返回 1</returns>
        /// Creator: chengym 2011-9-28
        public int QueryCase1(ICDTypes ICDType, QueryTypes queryType, ref DataSet ds)
        {
            //定义字符变量 ,存储查询主体SQL语句
            string strQuerySql = "";
            //定义字符变量, 存储查询条件
            ...
            try
            {
                ...
                switch (ICDType)
                {
                    case ICDTypes.ICD10:
                        //获取查询SQL语句
                        if (this.Sql.GetSql("Case.ICDDML.Query.ICD10.Base.Ds.Case1", ref strQuerySql) == -1)
                        {
                            this.Err = "获取SQL语句失败,索引:Case.ICDDML.Query.ICD10.Base.Ds.Case1";
                            return -1;
                        }
                        break;
                    case ICDTypes.ICD9:
        ...
        /// <returns></returns>
        public int InsertTempICD(Neusoft.HISFC.Models.HealthRecord.ICD info)
        {
            try
            {
                //定义字符变量 ，存储SQL语句
        ...
        /// <returns></returns>
        public List<Neusoft.HISFC.Models.HealthRecord.ICD> QueryTempICD(ICDTypes ICDType)
        {
            ...
            //定义动态数组 ,存储查询出的信息
            List<Neusoft.HISFC.Models.HealthRecord.ICD> arryList = new List<Neusoft.HISFC.Models.HealthRecord.ICD>();
            try
            {
                switch (ICDType)
                {
        ...
        /// <param name="ICDType"></param>
        /// <returns></returns>
        public List<Neusoft.HISFC.Models.HealthRecord.ICD> QueryTempICDAll(ICDTypes ICDType)
        {
        ...
        public ArrayList QueryDrgs(ICDTypes ICDType)
        {
            ...
            if (ICDType == Neusoft.HISFC.Models.HealthRecord.EnumServer.ICDTypes.ICD10)
            {
...
where   valid_state=fun_get_valid()";
            ...
            else if (ICDType == Neusoft.HISFC.Models.HealthRecord.EnumServer.ICDTypes.ICDOperation)
            {
...
where   valid_state=fun_get_valid()";
        ...
        /// <summary>
        /// 病案接口上传时需要判断一下是否标准ICD编码，如果不是不上传
        /// chengym 2017-4-17
        /// </summary>
        /// <param name="ICDType">ICD类型</param>
        /// <param name="ICDCode">ICD编码</param>
        /// <returns></returns>
        public Neusoft.HISFC.Models.HealthRecord.ICD QueryICDByCode(ICDTypes ICDType, string ICDCode)
        {
            //SQL语句
            string strQuerySql = "";
            switch (ICDType)
            {
                case ICDTypes.ICD10:
                    //获取查询SQL语句
                    if (this.Sql.GetSql("Case.ICDDML.Query.ICD10.BYCODE", ref strQuerySql) == -1)
                    {
                        this.Err = "获取SQL语句失败,索引:Case.ICDDML.Query.ICD10.BYCODE";
                        return null;
                    }
                    break;
                case ICDTypes.ICDOperation:
                    //获取查询SQL语句
                    if (this.Sql.GetSql("Case.ICDDML.Query.ICDOperation.BYCODE", ref strQuerySql) == -1)
                    {
                        this.Err = "获取SQL语句失败,索引:Case.ICDDML.Query.ICDOperation.BYCODE";
                        return null;
                    }
                    break;
            }
            ...
            {
                //格式化SQL语句
                strQuerySql = string.Format(strQuerySql, ICDCode);
            }
            catch (Exception ex)
            {
                this.Err = "SQL语句赋值出错!" + ex.Message;
