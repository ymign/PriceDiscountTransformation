...
namespace Neusoft.HISFC.BizLogic.RADT
{
	...
	public class OutPatient : Database
	{
        ...
        /// <returns></returns>
        public bool IsInputOutpatientEmr(string clinic_code)
        {
            string strSql = "";
            #region 接口说明

            //RADT.OutpatientEmr.IsInputOutpatientEmr.1
            //传入:门诊流水号
            //传出：是否已经录入门诊病历（bool）

            #endregion
            if (Sql.GetCommonSql("RADT.OutPatient.Get.2", ref strSql) == -1) return false;
            try
            {
        ...
        public int RegisterObservePatient(Neusoft.HISFC.Models.Registration.Register OutPatient)
        {
            ...
            //更新变更记录主表
            if (InpatentManager.SetShiftData(OutPatient.ID, Neusoft.HISFC.Models.Base.EnumShiftType.EB, "留观登记", OutPatient.DoctorInfo.Templet.Dept, OutPatient.DoctorInfo.Templet.Dept, false) >= 0)
            {
                return 0;
            }
                ...
                case EnumShiftType.EO:
                    {
                        ...
                        if (resultValue == 0)
                        {
                            resultValue = UpdateEmergencyObserve(OutPatient.ID, Neusoft.HISFC.Models.Base.EnumInState.R, Neusoft.HISFC.Models.Base.EnumInState.N, OutPatient.PVisit.InTime.ToString(), DateTime.MinValue.ToString(), "");
                        }
                        break;
                    }
