    ...
    public class UndrugztManager : DataBase
    {
        ...
        public int QueryUnDrugztDetail(string pcode,ref List<Neusoft.HISFC.Models.Fee.Item.UndrugComb> listzt)
        {
            ...
            while (this.Reader.Read())
            {
                ...
                {
                    zt.ItemRate = Neusoft.FrameWork.Function.NConvert.ToDecimal(Reader[15]);
                }
        ...
        public int SaveUndrugzt(List<Neusoft.HISFC.Models.Fee.Item.UndrugComb> lstUndrugzt)
        {
            ...
            for (int i = 0, j = lstUndrugzt.Count; i < j; i++)
            {
                ...
                if (this.ExecNoQuery(strInsert) > 0)
                {
                    this.UndrugztUpdateRecord(lstUndrugzt[i],"insert");
                }
                if (this.ExecNoQuery(strInsert) <= 0)
                {
                    string strUpdate = "";
                    ...
                    this.UndrugztUpdateRecord(lstUndrugzt[i], "update");
                    if (this.ExecNoQuery(strUpdate) <= 0)
                    {
                        return -1;
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public int UndrugztUpdateRecord(Neusoft.HISFC.Models.Fee.Item.UndrugComb comb,string type)
        {
            string strInsert = "";
            if (this.GetSQL("Fee.Itemzt.Info.InsertRecord", ref strInsert) == -1)
            {
                return -1;
            }
            try
            ...
            return this.ExecNoQuery(strInsert);
        }
