...
namespace Neusoft.HISFC.BizLogic.Manager
{
	...
	public class UserManager:DataBase 
	{
        ...
        #region 密码强度需求 {9E0A28E6-00C1-4118-A292-373A1C3E6622}
        /// <summary>
        /// 保存密码修改记录
        /// </summary>
        /// <param name="LoginID"></param>
        ...
        /// <param name="newPassword"></param>
        /// <returns></returns>
        public int SavePassWordRecord(string LoginID, string OldPassword, string newPassword)
        {
            string sql = "";
            if (this.GetSQL("Manager.UserManager.SavePassWordRecord", ref sql) == -1)
                return -1;
            sql = string.Format(sql, LoginID, OldPassword, newPassword);
        ...
        /// <returns></returns>
        public int CheckPwdRepeat(string LoginID, string password, int repeatTimes)
        {
            string sql = "";
            if (this.GetSQL("Manager.UserManager.CheckPwdRepeat", ref sql) == -1)
            {
                return -1;
            }
            sql = string.Format(sql, LoginID, password, repeatTimes);
            ...
            if (this.Reader.Read())
            {
                isRepeat = (Neusoft.FrameWork.Function.NConvert.ToInt32(this.Reader[0]) > 0);
            }
            this.Reader.Close();
            return (isRepeat ? 1 : 0);
        }
        ...
        /// <returns></returns>
        public int GetLastChangePwd(string LoginID, ref DateTime dtLastDate)
        {
            string sql = string.Empty;
            if (this.GetSQL("Manager.UserManager.GetLastChangePwd", ref sql) == -1)
                return -1;
            ...
            {
                this.Reader.Read();
                dtLastDate = Neusoft.FrameWork.Function.NConvert.ToDateTime(this.Reader[0]);
            }
            catch (Exception ex)
            {
                this.Err = ex.Message;
		...
		public ArrayList GetPersonGroupList(string PersonID)
		{
			...
			sql = string.Format(sql,PersonID,Neusoft.FrameWork.Management.Connection.Hospital.ID);
        ...
        /// <summary>
        /// 根据用户名查询最近一次错误登录的时间与当前时间差距多少分钟
        /// 超过6次输入错误 则需要等待5分钟才允许登录第二次
        /// </summary>
        /// <param name="empl_code">用户名</param>
        /// <returns></returns>
        public string GetLogonInfoByAccountID(string empl_code)
        {
            string strSql = "select   round((sysdate - max(t.login_date))*24*60,1)  from com_loginstatus t where t.empl_code='{0}'";
            ...
            try
            {
                if (this.Reader.Read())
                {
                    diffTime = this.Reader[0].ToString();
                }
            }
            catch (Exception ee)
            {
        ...
        /// <returns></returns>
        public ArrayList GetLoginSession(string emplCode)
        {
            string sql = @"select s.CLIENT_INFO,s.LOGON_TIME,s.PROGRAM,s.MACHINE,s.STATUS from COM_LOGINLOG l, v$session s where s.SID=l.sid  
 and s.SERIAL# =l.serial and   l.empl_code='{0}' and l.login_date>trunc(sysdate)";
            try
            {
            ...
            while (this.Reader.Read())
            {
                ...
                obj.ID = this.Reader[0].ToString();          //IP
                obj.Name = this.Reader[1].ToString();        //登录日期
                obj.Memo = this.Reader[2].ToString();        //执行的程序
                obj.User01 = this.Reader[3].ToString();      //机器
                obj.User02 = this.Reader[4].ToString();  //状态
                al.Add(obj);
            }
            this.Reader.Close();
            return al;
