...
namespace Neusoft.HISFC.BizLogic.Manager
{
    ...
    public class PactStatRelation : DataBase
    {
        ...
        /// <param name="Item">科室扩展属性类</param>
        /// <returns>0没有更新 1成功 -1失败</returns>
        public int InsertItem(Neusoft.HISFC.Models.Base.PactCompare Item)
        {
            string strSQL = "";
            //取插入操作的SQL语句
        ...
        //仿照上面的方法，只是换了传入的参数
        public int UpdateItem(Neusoft.HISFC.Models.Base.PactCompare Item)
        {
            string strSQL = "";
            //取更新操作的SQL语句
        ...
        public int SetItem(Neusoft.HISFC.Models.Base.PactCompare Item)
        {
            int parm;
            //执行更新操作
            parm = UpdateItem(Item);
        ...
        /// <returns>字符串数组</returns>
        private string[] myGetParmItem(Neusoft.HISFC.Models.Base.PactCompare Item)
        {
            string[] strParm ={   
								 Item.PactCode,
								 Item.PactHead,
        ...
        public string GetStatRelation(string pactCode, string groupID, bool isEmergency)
        {
            ...
            try
            {
                strSQL = string.Format(strSQL, pactCode, groupID, Neusoft.FrameWork.Function.NConvert.ToInt32(isEmergency));    //替换SQL语句中的参数。
            }
            catch (Exception ex)
            {
                this.Err = "格式化SQL语句时出错Manager.PactStatRelation.GetStatRelation.Select:" + ex.Message;
