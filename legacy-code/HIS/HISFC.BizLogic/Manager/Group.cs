...
namespace Neusoft.HISFC.BizLogic.Manager
{
    ...
    public class Group : DataBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ArrayList GetAllPCCOrderGroup(Neusoft.HISFC.Models.Base.ServiceTypes t)
        {
            string sql = "", sqlWhere = "";
            ...
            while (this.Reader.Read())
            {
                ...
                try
                {
                    ...
                    try//时间间隔
                    {
                        ...
                        {
                            info.Item.ItemType = Neusoft.HISFC.Models.Base.EnumItemType.Drug;
                        }
                        else
                        {
                            info.Item.ItemType = Neusoft.HISFC.Models.Base.EnumItemType.UnDrug;
                        }
                    }
                    catch { }
            ...
            while (this.Reader.Read())
            {
                ...
                try
                {
                    ...
                    try//时间间隔
                    {
                        ...
                        info.Item.Name = this.Reader[21].ToString();
                        //if ("P,PCC,PCZ".Contains(info.Item.SysClass.ID.ToString()))
                        if (info.Item.ID.StartsWith("Y") || info.Item.ID.StartsWith("H"))
                        {
                            info.Item.ItemType = Neusoft.HISFC.Models.Base.EnumItemType.Drug;
                        }
                        else
        ...
        public int UpdateIsSelectAll(string groupID, bool isSelectAll)
        {
            ...
            try
            {
                return this.ExecNoQuery(strSql, groupID, Neusoft.FrameWork.Function.NConvert.ToInt32(isSelectAll).ToString());
            }
            catch (Exception ex)
            {
                this.ErrCode = ex.Message;
