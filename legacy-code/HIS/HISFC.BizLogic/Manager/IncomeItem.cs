...
//			this.Reader = null;
		...
		public int GetItemList(string deparment, string incomeItem, ArrayList alConstant)
		{
			...
			intReturn = this.GetSQL("Neusoft.HISFC.Management.Manager.IncomeItem.GetItemList.Where", ref WHERE);
			if (intReturn == -1)
			{
				this.Err = "获取SQL语句失败！" + this.Err;
				return -1;
...
//			this.Reader = null;
		...
		public int JudgeIncomeItemExist(string department)
		{
			...
			intReturn = this.GetSQL("Neusoft.HISFC.Management.Manager.JudgeIncomeItemExist.Where", ref WHERE);
			if (intReturn == -1)
			{
				this.Err = "获取SQL语句失败！" + this.Err;
				return -1;
...
//			this.Reader = null;
		...
		public int Delete(Neusoft.HISFC.Models.Base.Const c, int intOperate)
		{
			...
			{
				case 1:
					intReturn = this.GetSQL("Neusoft.HISFC.Management.Manager.IncomeItem.Delete.Where.IncomeItem", ref WHERE);
					if (intReturn == -1)
					{
						this.Err = "获取SQL语句失败！" + this.Err;
						return -1;
					...
					break;
				case 2:
					intReturn = this.GetSQL("Neusoft.HISFC.Management.Manager.IncomeItem.Delete.Where.FeeItem", ref WHERE);
					if (intReturn == -1)
					{
						this.Err = "获取SQL语句失败！" + this.Err;
						return -1;
