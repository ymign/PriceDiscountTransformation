...
namespace Neusoft.HISFC.BizLogic.Order.Medical
{
    ...
    public class Ability : Neusoft.FrameWork.Management.Database
    {
        ...
        /// <param name="itemID">药品代码</param>
        /// <returns>成功 字符串 失败 null</returns>
        private string GetControlsLevelByID(string itemID)
        {
            return this.GetParamsBySql("Order.Allergy.GetControlsLevelByID", itemID);
        }
        ...
        /// <param name="itemID">药品代码</param>
        /// <returns>成功 字符串 失败 null</returns>
        private string GetControlsDrugByID(string itemID)
        {
            return this.GetParamsBySql("Order.Allergy.GetControlsDrugByID", itemID);
        }
        ...
        /// <summary>
        /// 通过权限ID和类型验证权限(抗菌药物等级)
        /// </summary>
        /// <param name="emplID">人员代码</param>
        /// <param name="popedomID">权限代码</param>
        /// <returns>成功 "1" 失败 "-1" 无权限 "0"</returns>
        public string CheckByTypeLevel(string emplID, string popedomType)
        {
            return this.GetParamsBySql("Order.Allergy.CheckPopedomByTypeLevel", emplID, popedomType);
        }

        /// <summary>
        /// 判断门诊医生是否有开立特殊药品的权限
        /// 维护界面位于：门诊药房管理--基本信息维护--特限药品维护
        /// </summary>
        /// <param name="doctor"></param>
        /// <param name="?"></param>
        /// <returns></returns>
        public string IsHaveSpecDrugAuthority(string doctor, string drugCode, string speType) 
        {
            return this.GetParamsBySql("Order.Allergy.IsHaveSpecDrugAuthority", doctor, drugCode, speType);
        }
        ...
        public int CheckPopedom(string emplCode, string itemCode, string sysClass, bool isAllowNonRight, ref string failCause)
        {
            ...
            if (sysClass.Substring(0, 1) == "P")
            {
                ...
                else
                {
                    ...
                    string cancerdrugs = this.GetControlsDrugByID(itemCode);
                    // {EAB5C1BF-2964-4ee3-89DA-45B3C585CE6E} 抗菌药物处方权限
                    string drugslevel = this.GetControlsLevelByID(itemCode);                   
                    if (!string.IsNullOrEmpty(drugslevel))
                    {
                        checkResult = this.CheckByTypeLevel(emplCode, "6");
                        if (string.IsNullOrEmpty(checkResult))
                        {
                            failCause = "您没有抗菌药物处方权限！如有疑问请联系信息科！";

                            return 0;
                        }
                        else if (int.Parse(drugslevel) > int.Parse(checkResult))
                        {
                            failCause = "您的抗菌药物处方权限不够！如有疑问请联系信息科！";
                    ...
                    else if (cancerdrugs == null)
                    {
                        failCause = "获取药品抗肿瘤信息信息失败！请联系信息科！";

                        return -1;
                    }
                    else
                    {
                        if (drugQuality == "" && cancerdrugs == "")
                        {
                            return 1;
                        }

                        if (drugQuality == null || cancerdrugs == null)
                        {
                            failCause = "获取药品性质信息失败！请联系信息科！";
                        ...
                        {
                            checkResult = this.CheckByType(emplCode, "1", drugQuality);
                            string control = this.CheckByType(emplCode, "5", cancerdrugs);
                            if (checkResult == "1" && control == "1")
                            {
                                return 1;
                            }
                            else if (checkResult == "0" || control == "0")
                            {
                                failCause = "您没有此药品性质的处方权限！如有疑问请联系信息科！";
        ...
        /// <param name="emplID">人员代码</param>
        /// <returns>成功 权限实体列表 失败 null</returns>
        public List<Neusoft.HISFC.Models.Order.Medical.Popedom> QueryPopedomAll()
        {
            string sql = string.Empty;
