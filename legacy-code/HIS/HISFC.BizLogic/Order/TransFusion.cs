...
namespace Neusoft.HISFC.BizLogic.Order
{
	...
	public class TransFusion:Neusoft.FrameWork.Management.Database
	{
        ...
        /// <param name="usageCode"></param>
        /// <returns></returns>
        public int InsertTransFusionTPD(string nurseCode, string usageCode)
        {
            #region "接口"
            //传入：0 病区编码 1用法编码 2 操作员
            //传出：0
            ...
            if (this.Sql.GetCommonSql("Order.TransFusion.InsertItem.TPD", ref strSql) == -1)
            {
                this.Err = "没有找到Order.TransFusion.InsertItem.TPD";
                return -1;
            }
            try
            {
        ...
        /// <param name="usageCode"></param>
        /// <returns>-1 错误 0 没有找到记录 >0 记录行数</returns>
        public int DeleteTransFusionTPD(string nurseCode, string usageCode)
        {
            string strSql = "";
            ...
            //传出：0
            #endregion
            if (this.Sql.GetCommonSql("Order.TransFusion.DeleteItem.TPD", ref strSql) == -1)
            {
                this.Err = "没有找到Order.TransFusion.DeleteItem.TPD";
        ...
        /// <param name="nurseCode"></param>
        /// <returns></returns>
        public ArrayList QueryTransFusionTPD(string nurseCode)
        {
            ArrayList al = new ArrayList();
            string strSql = "";
            //Order.TransFusion.Select.1
            //传入：0  NurseCode
            //传出:属于该项目或用法的附材
            if (this.Sql.GetCommonSql("Order.TransFusion.Select.TPD", ref strSql) == 0)
            {
                if (this.ExecQuery(strSql, nurseCode) == -1) return null;
                while (this.Reader.Read())
                {
            ...
            {
                this.Err = "没有找到Order.TransFusion.Select.TPD";
                if (!this.Reader.IsClosed)
                    this.Reader.Close();
                return null;
            }
