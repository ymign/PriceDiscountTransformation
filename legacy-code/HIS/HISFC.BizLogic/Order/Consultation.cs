...
namespace Neusoft.HISFC.BizLogic.Order
{
    ...
    public class Consultation : Neusoft.FrameWork.Management.Database
    {
        ...
        // {BA131C95-2F28-4e99-85DD-8D43F8416EFA} 
        /// <summary>
        /// 获得会诊列表（从电子病历获取）
        /// </summary>
        /// <param name="InpatientNo"></param>
        /// <returns></returns>
        public ArrayList QueryConsulationNew(string InpatientNo)
        {
            string strSql = "";
            //Order.Consultation.SelectItemFromEMR.1
            //传入：0  InpatientNo
            //传出:会诊
            if (this.Sql.GetCommonSql("Order.Consultation.SelectItemFromEMR.1", ref strSql) == -1)
            {
                this.Err = this.Sql.Err;
                return null;
            }
