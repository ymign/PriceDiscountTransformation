...
namespace Neusoft.HISFC.BizLogic.Registration
{
	...
	public class Booking : Neusoft.FrameWork.Management.Database
	{
		...
		public ArrayList QueryBase(string sql)
		{
			...
			try
			{
				while(this.Reader.Read())
				{
                    ...
                    //{
                    //    this.objBooking.Age = this.Reader[24].ToString();
                    //}

                    //预约信息
                    if (this.Reader.FieldCount > 24)
                    {
                        this.objBooking.BookingTypeId = this.Reader[24] != DBNull.Value ? this.Reader[24].ToString() : string.Empty;
                        //this.objBooking.BookingTypeName = this.Reader[26] != DBNull.Value ? this.Reader[26].ToString() : string.Empty;
                    }

                    //全院序号和当前序号
                    if (this.Reader.FieldCount > 27)
                    {
                        this.objBooking.OrderNO = Neusoft.FrameWork.Function.NConvert.ToInt32(this.Reader[27].ToString());
                        this.objBooking.DoctorInfo.SeeNO = Neusoft.FrameWork.Function.NConvert.ToInt32(this.Reader[28].ToString());
                    }
        ...
        /// <returns></returns>
        public Neusoft.HISFC.Models.Registration.Booking GetByCardNo(string cardNo)
        {
            try
            {
                string sql = "", where = "";
                ...
                if (this.Sql.GetCommonSql("Registration.Booking.Query.1", ref sql) == -1) return null;
                if (this.Sql.GetCommonSql("Registration.Booking.QueryByCardNo", ref where) == -1) return null;

                sql = sql + " " + where;
                sql = string.Format(sql, cardNo);

                if (this.QueryBase(sql) == null) return null;
                if (this.al == null) return null;
                else if (this.al.Count == 0) return new Neusoft.HISFC.Models.Registration.Booking();
                else return this.al[0] as Neusoft.HISFC.Models.Registration.Booking;
            }
            catch (Exception ex)
            {
                this.Err = ex.Message;
