using System;


namespace Neusoft.HISFC.Models.HealthRecord
{


    /// <summary>
    /// CasOperationDetail 的摘要说明。
    /// </summary>
    [Serializable]
    public class OperationDetail
    {
        public OperationDetail()
        {
            //
            // TODO: 在此处添加构造函数逻辑
            //
        }
        #region 私有变量
        //私有字段
        private string myInpatientNO;
        private string myHappenNO;
        private DateTime myOperationDate;
        private Neusoft.FrameWork.Models.NeuObject myOperationInfo = new Neusoft.FrameWork.Models.NeuObject();
        private string myOperationKind;
        // {89C4E8A4-DA2B-401f-A0A9-524D7B2D25DF}
        private string addFlag;  //附加手术标记
        private string myMarcKind;
        private string myNickKind;
        private string myCicaKind;
        private string operationName1;
        private Neusoft.FrameWork.Models.NeuObject myFirDoctInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject mySecDoctInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject myThrDoctInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject myNarcDoctInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject myFourDoctInfo = new Neusoft.FrameWork.Models.NeuObject();
        private string myOpbOpa;
        private int myBeforeOperDays;
        private string myStatFlag;
        private DateTime myInDate;
        private DateTime myOutDate;
        private DateTime myDeatDate;
        private Neusoft.FrameWork.Models.NeuObject myOperationDeptInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject myOutDeptInfo = new Neusoft.FrameWork.Models.NeuObject();
        private Neusoft.FrameWork.Models.NeuObject myOutICDInfo = new Neusoft.FrameWork.Models.NeuObject();
        private string mySYNDFlag;
        private DateTime myOperDate;
        private string operType;
        private string operationNO;
        #endregion

        #region 属性

        /// <summary>
        /// 住院流水号
        /// </summary>
        public string InpatientNO
        {
            get { return myInpatientNO; }
            set { myInpatientNO = value; }
        }

        /// <summary>
        /// 发生序号
        /// </summary>
        public string HappenNO
        {
            get { return myHappenNO; }
            set { myHappenNO = value; }
        }

        /// <summary>
        /// 手术日期
        /// </summary>
        public DateTime OperationDate
        {
            get { return myOperationDate; }
            set { myOperationDate = value; }
        }

        /// <summary>
        /// 手术信息ID 手术编码 Name手术姓名
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject OperationInfo
        {
            get { return myOperationInfo; }
            set { myOperationInfo = value; }
        }

        /// <summary>
        /// 手术种类
        /// </summary>
        public string OperationKind
        {
            get { return myOperationKind; }
            set { myOperationKind = value; }
        }

        // {89C4E8A4-DA2B-401f-A0A9-524D7B2D25DF}
        /// <summary>
        /// 手术种类
        /// </summary>
        public string AddFlag
        {
            get { return addFlag; }
            set { addFlag = value; }
        }

        /// <summary>
        /// 麻醉方式
        /// </summary>
        public string MarcKind
        {
            get { return myMarcKind; }
            set { myMarcKind = value; }
        }

        // {5EEFA638-9971-4c89-B94A-B26F326891D9}
        /// <summary>
        /// 手术及操作名称1
        /// </summary>
        public string OperationName1
        {
            get { return operationName1; }
            set { operationName1 = value; }
        }

        /// <summary>
        /// 切口种类
        /// </summary>
        public string NickKind
        {
            get { return myNickKind; }
            set { myNickKind = value; }
        }

        /// <summary>
        /// 愈合种类
        /// </summary>
        public string CicaKind
        {
            get { return myCicaKind; }
            set { myCicaKind = value; }
        }

        /// <summary>
        /// 手术医师信息1 ID 医生编码 Name 医生姓名
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject FirDoctInfo
        {
            get { return myFirDoctInfo; }
            set { myFirDoctInfo = value; }
        }

        /// <summary>
        /// 手术医师信息2 ID 医生编码 Name 医生姓名
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject FourDoctInfo
        {
            get { return myFourDoctInfo; }
            set { myFourDoctInfo = value; }
        }

        /// <summary>
        /// I助医师信息 ID 医生编码 Name 医生姓名
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject SecDoctInfo
        {
            get { return mySecDoctInfo; }
            set { mySecDoctInfo = value; }
        }

        /// <summary>
        /// II助医师信息 ID 医生编码 Name 医生姓名
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject ThrDoctInfo
        {
            get { return myThrDoctInfo; }
            set { myThrDoctInfo = value; }
        }

        /// <summary>
        /// 麻醉医师信息
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject NarcDoctInfo
        {
            get { return myNarcDoctInfo; }
            set { myNarcDoctInfo = value; }
        }

        /// <summary>
        /// 术前_后符合
        /// </summary>
        public string OpbOpa
        {
            get { return myOpbOpa; }
            set { myOpbOpa = value; }
        }

        /// <summary>
        /// 术前住院天数
        /// </summary>
        public int BeforeOperDays
        {
            get { return myBeforeOperDays; }
            set { myBeforeOperDays = value; }
        }

        /// <summary>
        /// 统计标志
        /// </summary>
        public string StatFlag
        {
            get { return myStatFlag; }
            set { myStatFlag = value; }
        }

        /// <summary>
        /// 入科日期
        /// </summary>
        public DateTime InDate
        {
            get { return myInDate; }
            set { myInDate = value; }
        }

        /// <summary>
        /// 出院日期
        /// </summary>
        public DateTime OutDate
        {
            get { return myOutDate; }
            set { myOutDate = value; }
        }

        /// <summary>
        /// 死亡日期
        /// </summary>
        public DateTime DeatDate
        {
            get { return myDeatDate; }
            set { myDeatDate = value; }
        }

        /// <summary>
        /// 手术科室信息ID 科室编码 Name 科室名称
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject OperationDeptInfo
        {
            get { return myOperationDeptInfo; }
            set { myOperationDeptInfo = value; }
        }

        /// <summary>
        /// 出院病房信息 ID 科室编码 Name 科室名称
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject OutDeptInfo
        {
            get { return myOutDeptInfo; }
            set { myOutDeptInfo = value; }
        }

        /// <summary>
        /// 出院主诊断ICD信息 ID信息编码 Name 诊断名称
        /// </summary>
        public Neusoft.FrameWork.Models.NeuObject OutICDInfo
        {
            get { return myOutICDInfo; }
            set { myOutICDInfo = value; }
        }

        /// <summary>
        /// 是否合并症
        /// </summary>
        public string SYNDFlag
        {
            get { return mySYNDFlag; }
            set { mySYNDFlag = value; }
        }

        /// <summary>
        /// 操作时间
        /// </summary>
        public DateTime OperDate
        {
            get { return myOperDate; }
            set { myOperDate = value; }
        }

        /// <summary>
        /// 手术种类
        /// </summary>
        public string OperType
        {
            get
            {
                return operType;
            }
            set
            {
                operType = value;
            }
        }

        /// <summary>
        /// 手术序列号
        /// </summary>
        public string OperationNO
        {
            get
            {
                return operationNO;
            }
            set
            {
                operationNO = value;
            }
        }
        #endregion

        #region 公用函数


        public new OperationDetail Clone()
        {
            OperationDetail OperationDetailClone = base.MemberwiseClone() as OperationDetail;

            OperationDetailClone.myFirDoctInfo = this.myFirDoctInfo;
            OperationDetailClone.myNarcDoctInfo = this.myNarcDoctInfo;
            OperationDetailClone.myOperationDeptInfo = this.myOperationDeptInfo;
            OperationDetailClone.myOperationInfo = this.myOperationInfo;
            OperationDetailClone.myOutDeptInfo = this.myOutDeptInfo;
            OperationDetailClone.myOutICDInfo = this.myOutICDInfo;
            OperationDetailClone.mySecDoctInfo = this.mySecDoctInfo;
            OperationDetailClone.myThrDoctInfo = this.myThrDoctInfo;

            return OperationDetailClone;

        }

        #endregion
    }
}
