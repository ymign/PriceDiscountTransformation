...
namespace Neusoft.SOC.Local.OutpatientFee.ZhuHai.Zdwy.IOutpatientInfomation
{
    public partial class ucPatientInfo : UserControl, Neusoft.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientInfomation
    {
        ...
        /// <summary>
        /// 处方类型编码（0或空普通，1急救抢救，2自费）
        /// </summary>
        protected string recipeTypeCode = string.Empty;
        ...
        private Hashtable hsPact = null;

        private Hashtable hsPactInfo = null;
        ...
        public Neusoft.HISFC.Models.Registration.Register PatientInfo
        {
            ...
            set
            {
                ...
                if (patientInfo != null)
                {
                    ...
                    if (patientInfo.Pact.ID == "258")
                    {
                        this.cbZZQ.Checked = true;
                    }

                    //this.cmbDoct.Tag = patientInfo.DoctorInfo.Templet.Doct.ID;

                    #region add by lijp 2016-07-13, 中大五院存在护士开医嘱的情况，这里的doctList只加载医生，但是是护士开单给cmbDoct赋值时赋值不上去。
                    //需要联网结算 必须要选择医生  2016-8-8 usopp
                    //Neusoft.HISFC.Models.Base.Employee empl = new Neusoft.HISFC.Models.Base.Employee();
                    //empl = Neusoft.SOC.HISFC.BizProcess.Cache.Common.EmployeeHelper.GetObjectFromID(this.patientInfo.SeeDoct.ID) as Neusoft.HISFC.Models.Base.Employee;

                    //if (!string.IsNullOrEmpty(this.patientInfo.SeeDoct.ID) && empl.EmployeeType.ID.ToString() != "D")
                    //{
                    //    this.cmbDoct.ClearItems();
                    //    cmbDoct.AddItems(doctList);
                    //    cmbDoct.AddItems(NurseList);
                    //}
                    //else
                    //{
                    //    this.cmbDoct.ClearItems();
                    ...
                    cmbDoct.AddItems(doctList);
                    //cmbDoct.AddItems(NurseList);
                    if (!string.IsNullOrEmpty(this.patientInfo.SeeDoct.ID))
                    {
                        Neusoft.HISFC.Models.Base.Employee empl = new Neusoft.HISFC.Models.Base.Employee();
                        empl = Neusoft.SOC.HISFC.BizProcess.Cache.Common.EmployeeHelper.GetObjectFromID(this.patientInfo.SeeDoct.ID) as Neusoft.HISFC.Models.Base.Employee;
                        ...
                        else
                        {
                            ...
                            {
                                ArrayList al = (ArrayList)doctList.Clone();
                                al.Add(empl);
                                this.cmbDoct.ClearItems();
                                cmbDoct.AddItems(al);
                                //this.cmbDoct.Items.AddRange(new object[] { empl });
                                //this.cmbDoct.Tag = this.patientInfo.SeeDoct.ID;
                            }
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(patientInfo.PatientType))
                    {
                        Neusoft.HISFC.Models.Base.PactInfo pact = this.managerIntegrate.GetPactUnitInfoByPactCode(patientInfo.Pact.ID);
                        this.cmbPatientType.Tag = pact.PatientType.ID;
                    }
                    else
                    {
                        this.cmbPatientType.Tag = patientInfo.PatientType;
                    }
        ...
        public void Clear()
        {
            ...
            this.cmbPatientType.Tag = null;
            this.cmbPatientType.Text = string.Empty;
            this.cmbPact.SelectedIndexChanged -= new EventHandler(cmbPact_SelectedIndexChanged);
            ...
            this.tbCardNO.Focus();
            this.neutesSFBZ.Text = string.Empty;
            this.cbZZQ.Checked = false;
        }
        ...
        private int InitControlParams()
        {
            ...
            //门诊单位体检卡号第一位标志
            this.dwtjcode = this.controlParamIntegrate.GetControlParam<string>("MZTJ01", false, "1");
        ...
        public int Init()
        {
            try
            {
                ...
                {
                    //初始化 挂号科室
                    this.regDeptList = this.managerIntegrate.GetDepartmentWithHosCode(Neusoft.HISFC.Models.Base.EnumDepartmentType.C, Neusoft.FrameWork.Management.Connection.Hospital.ID);//.GetDepartment(Neusoft.HISFC.Models.Base.EnumDepartmentType.C);
                    ...
                    //可以添加住院科室  xingz 待讨论
                    ArrayList alDeptInPatient = this.managerIntegrate.GetDepartmentWithHosCode(Neusoft.HISFC.Models.Base.EnumDepartmentType.I, Neusoft.FrameWork.Management.Connection.Hospital.ID);//.GetDepartment(Neusoft.HISFC.Models.Base.EnumDepartmentType.I);
                    this.regDeptList.AddRange(alDeptInPatient);
                    //加终端科室
                    ArrayList alDeptEndPoint = this.managerIntegrate.GetDepartmentWithHosCode(Neusoft.HISFC.Models.Base.EnumDepartmentType.T, Neusoft.FrameWork.Management.Connection.Hospital.ID);//.GetDepartment(Neusoft.HISFC.Models.Base.EnumDepartmentType.T);
                    this.regDeptList.AddRange(alDeptEndPoint);
                }
                else
                {
                    this.regDeptList = this.managerIntegrate.GetDeptmentAllValidWithHosCode(Neusoft.FrameWork.Management.Connection.Hospital.ID);//.GetDeptmentAllValid();
                    if (this.regDeptList == null)
                    {
                        MessageBox.Show("初始化挂号科室出错!" + this.managerIntegrate.Err);
                ...
                NurseList = this.managerIntegrate.QueryEmployee(Neusoft.HISFC.Models.Base.EnumEmployeeType.N);
                if (doctList == null)
                {
        ...
        private int InitPact()
        {
            ...
            if (!isTransferTreat)
            {
                pactList = this.pactManager.QueryPactUnitOutPatientWithHosCodeAndRemoveMZSF(Neusoft.FrameWork.Management.Connection.Hospital.ID);
            }
            else
            {
                pactList = this.pactManager.QueryPactUnitOutPatient();
                //pactList = this.pactManager.QueryPactUnitAll();

                //pactList = this.pactManager.QueryPactUnitOutPatientWithHosCode("CORE_HIS50");
            }
            if (pactList == null)
            {
            ...
            this.hsPact = new Hashtable();
            this.hsPactInfo = new Hashtable();
            foreach (Neusoft.HISFC.Models.Base.PactInfo pactUnit in pactListFinal)
            {
                if (this.hsPact.ContainsKey(pactUnit.PatientType.ID))
                {
                    ArrayList altemp = this.hsPact[pactUnit.PatientType.ID] as ArrayList;
                    altemp.Add(pactUnit);
                }
                else
                {
                    ...
                    altemp.Add(pactUnit);
                    this.hsPact.Add(pactUnit.PatientType.ID, altemp);
                }
                if (!this.hsPactInfo.ContainsKey(pactUnit.ID))
                {
                    hsPactInfo.Add(pactUnit.ID,pactUnit);
                }
            }

            ArrayList alPateintType = this.managerIntegrate.GetConstantList("PatientType");

            if (alPateintType == null)
            {
                alPateintType = new ArrayList();
            }
        ...
        public void SetRegInfoCanModify(FeeItemList feeItemList, bool isCanModify)
        {
            ...
            if (string.IsNullOrEmpty(feeItemList.Order.Patient.PatientType))
            {
                Neusoft.HISFC.Models.Base.PactInfo pactInfo = this.hsPactInfo[feeItemList.Order.Patient.Pact.ID] as Neusoft.HISFC.Models.Base.PactInfo;
                if (pactInfo == null)
                {
                    pactInfo = this.hsPactInfo["1"] as Neusoft.HISFC.Models.Base.PactInfo;
                }
                this.cmbPatientType.Tag = pactInfo.PatientType.ID;

            }
            else
            {
                this.cmbPatientType.Tag = feeItemList.Order.Patient.PatientType;
            }

            if (!string.IsNullOrEmpty(feeItemList.Order.Patient.Pact.ID) && this.cmbPact.Tag.ToString() != "258")
            {
        ...
        public void GetRegInfo()
        {
            ...
            this.patientInfo.PatientType = this.cmbPatientType.Tag.ToString();
            this.patientInfo.Pact.ID = this.cmbPact.Tag.ToString();
            this.patientInfo.Pact.Name = this.cmbPact.Text;
            //this.patientInfo.Pact = this.GetPactInfoByPactCode(this.cmbPact.Tag.ToString());
            ...
            this.patientInfo.FeeRemark = this.neutesSFBZ.Text;
        ...
        public void SetRegInfo()
        {
            ...
            this.cmbPatientType.Tag = this.patientInfo.PatientType;
        ...
        public bool IsPatientInfoValid()
        {
            ...
            if (this.cmbPatientType.Text.Trim().Length <= 0 || this.cmbPatientType.Tag == null)
            {
                MessageBox.Show(Language.Msg("请选择患者类别!"));
            ...
            if (this.cmbPact.Text.Trim() != "公费")
            {
                if (this.patientInfo.Pact.PayKind.ID == "03" && this.tbJZDNO.Text.Trim() == string.Empty
                    && ((Neusoft.HISFC.Models.Base.Employee)this.outpatientManager.Operator).EmployeeType.ID.ToString() == Neusoft.HISFC.Models.Base.EnumEmployeeType.F.ToString())
                {
                    MessageBox.Show(Language.Msg("请输入记账单号!"));
                    this.tbJZDNO.Text = string.Empty;
                    this.tbJZDNO.Focus();
        ...
        {
            this.recipeTypeCode = string.Empty;
            ...
            for (int i = 0; i < sortList.Count; i++)
            {
                ...
                {
                    RecipeType = Function.GetRecipeType(tempFeeItemRowOne.Order.Combo.ID);
                    if (i < 1)
                    {
                        if (RecipeType.Contains("自费"))
                        {
                            this.recipeTypeCode = "2";
                            this.cmbPatientType.Tag = "1";
                            this.cmbPatientType.Text = "普通人群";
                            this.cmbPact.Tag = "1";
                    ...
                    this.fpRecipeSeq_Sheet1.Cells[i, 1].Text = RecipeType+objHelperDept.GetName(tempFeeItemRowOne.RecipeOper.Dept.ID);
                }
                else
                {
        ...
        public void CardNoEnter(string cardNO)
        {
            if (InputedCardAndEnter != null)
            {
                InputedCardAndEnter(cardNO, cardNO, this.tbCardNO.Location, this.tbCardNO.Height);
            }
        ...
        /// <param name="e"></param>
        public void tbCardNO_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else //正常输入患者门诊卡号情况
                {
                    ...
                    else
                    {
                        ...
                        if (cardNO.Length != 10 || (!cardNO.StartsWith(this.noRegFlagChar) && !cardNO.StartsWith(this.dwtjcode)))
                        {
        ...
        private void cmbPatientType_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                this.NextFocus(this.cmbPatientType);
            }
            else if (e.KeyCode == Keys.PageUp)
            {
                if (this.isCanModifyPatientInfo)
            ...
            if (this.fpRecipeSeq_Sheet1.RowCount > 0)
            {
                ...
                try
                {
                    foreach (FeeItemList f in (ArrayList)fpRecipeSeq_Sheet1.Rows[row].Tag)
                    {
                        ...
                        RecipeType = Function.GetRecipeType(f.Order.Combo.ID);
                    }

                    this.fpRecipeSeq_Sheet1.Cells[row, 1].Text = RecipeType+this.patientInfo.DoctorInfo.Templet.Dept.Name;
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
        ...
        private void cmbPact_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //体检中心收费模式下，10记账，12职工医保这两个合同单位切换直接返回
            if (this.cmbPact.Tag.ToString() == "10" || this.cmbPact.Tag.ToString() == "12")
            {
                this.cmbPact.Focus();
                return;
            }
            ...
            //判断自费或者医保单位在门诊不走医保流程
            if (this.patientInfo.Pact.PayKind.ID == "01" || this.patientInfo.Pact.IsUseInOutPatientFee == false)//自费
            {
                this.cmbClass.Enabled = false;
                this.tbMCardNO.Enabled = false;
                this.cmbRebate.Enabled = false;
        ...
        private void fpRecipeSeq_ButtonClicked(object sender, EditorNotifyEventArgs e)
        {
            if (e.Column == 0)//点击选择框
            {
                string oldCombNo = Function.GetCombNoByRecipeSeq(this.recipeSequence);
                string oldRecipeType = Function.GetRecipeType(oldCombNo);
                string newCombNo = Function.GetCombNoByRecipeSeq(this.fpRecipeSeq_Sheet1.Cells[e.Row, 3].Tag.ToString());
                string newRecipeType = Function.GetRecipeType(newCombNo);
                ...
                if (selectedCount == 1)
                {
                    ...
                    string RecipeType = Function.GetRecipeType(newCombNo);
                    if (newRecipeType.Contains("自费"))
                    {
                        this.recipeTypeCode = "2";
                        this.cmbPatientType.Tag = "1";
                        this.cmbPatientType.Text = "普通人群";
                    ...
                    else if (newRecipeType.Contains("门诊共济(谈判药)"))
                    {
                        this.cmbPact.Tag = "199";
                        this.cmbPact.Text = "门诊谈判药（无病种）";
                    }
                    else
                    {
                        this.cmbPact.Tag = this.patientInfo.Pact.ID;
                        this.cmbPact.Text = this.patientInfo.Pact.Name;
                ...
                {
                    if (oldRecipeType != newRecipeType)
                    {
                        this.fpRecipeSeq_Sheet1.Cells[e.Row, 0].Value = false;
                        MessageBox.Show("不同类型处方单请分开结算！");
                        return;
                    }
                    if (newRecipeType.Contains("自费"))
                    {
        ...
        private void fpRecipeSeq_CellClick(object sender, CellClickEventArgs e)
        {
            ...
            if (e.Column != 0)
            {
                ...
                this.recipeSequence = this.fpRecipeSeq_Sheet1.Cells[e.Row, 3].Tag.ToString();
                string combNo = Function.GetCombNoByRecipeSeq(this.recipeSequence);
                string RecipeType = Function.GetRecipeType(combNo);
                if (RecipeType.Contains("自费"))
                {
                    this.recipeTypeCode = "2";
                    this.cmbPatientType.Tag = "1";
        ...
        private void tbAge_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                try
                {
                    //没有出生日期才需要根据年龄反推出生日期  这里导致了患者基本信息的出生日期错误，主索引生成出错 chengym 20170110
                    if (this.patientInfo.Birthday == System.DateTime.MinValue)
                    {
                        this.patientInfo.Birthday = this.GetAge(this.outpatientManager.GetDateTimeFromSysDateTime(), age, month, day);
                    }
                    //出生年月
                    System.DateTime dateBirthday1 = new DateTime();
                    if (this.patientInfo.IDCard != null && this.patientInfo.IDCard.Trim().Length == 18)  //18位身份证号
                    {
                        string strTemp = this.patientInfo.IDCard.Trim();
                        //获得出生日期
                        try
                        {
                            dateBirthday1 = Convert.ToDateTime(strTemp.Substring(6, 4) + "-" + strTemp.Substring(10, 2) + "-" + strTemp.Substring(12, 2));
                        }
                        catch
                        {
                            dateBirthday1 = System.DateTime.MinValue;
                        }
                        this.patientInfo.Birthday = dateBirthday1;
                    }
                    else if (this.patientInfo.IDCard != null && this.patientInfo.IDCard.Trim().Length == 15)
                    {
                        string strTemp = this.patientInfo.IDCard.Trim();
                        //获得出生日期
                        try
                        {
                            dateBirthday1 = Convert.ToDateTime("19" + strTemp.Substring(6, 2) + "-" + strTemp.Substring(8, 2) + "-" + strTemp.Substring(10, 2));
                        }
                        catch
                        {
                            dateBirthday1 = System.DateTime.MinValue;
        ...
        public void CustomMethod()
        {
            ...
            if (!string.IsNullOrEmpty(patientInfo.Card.ID))
                this.CardNoEnter(this.patientInfo.Card.ID);
        }
        /// <summary>
        /// 获取开方科室
        /// </summary>
        ...
        {
            this.cmbPact.ClearItems();
            this.cmbPact.SelectedIndexChanged -= new EventHandler(cmbPact_SelectedIndexChanged);
            this.cmbPact.Tag = null;
            this.cmbPact.Text = "";
            this.cmbPact.SelectedIndexChanged += new EventHandler(cmbPact_SelectedIndexChanged);
            if (this.hsPact == null || string.IsNullOrEmpty(this.cmbPatientType.Tag.ToString()))
            {
                return;
            }

            ArrayList alTemp = this.hsPact[this.cmbPatientType.Tag.ToString()] as ArrayList;
            this.cmbPact.AddItems(alTemp);
            bool IsXJ = false;
            for (int i = 0; i < alTemp.Count; i++)
            {
                if (alTemp[i].ToString() == null)
                    continue;
                if (alTemp[i].ToString() == "现金")
                    IsXJ = true;
            }
            if (this.recipeTypeCode == "2")//判断是否是自费处方
            {
                if (IsXJ)
                {
                    this.cmbPact.Tag = "1";
