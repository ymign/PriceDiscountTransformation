...
1. 按项目、场景、部位、时间匹配规则
2. 双单位换算（如已配置）
3. 数量限制 / 时间窗数量限制 / 同组互斥（如已配置）
4. 公式折价（如已配置；FIN_DISCOUNT_FEE 使用限制后的 FinalQty）
5. 金额下限
6. 金额上限 / TOPPRICE 封顶
7. 同手术封顶
8. 子项加收 / 附加项目
9. 超出部分 → 0元兜底（不是整单，不是拒单）
...
- **HIS 兼容顺序：** 旧 HIS 正式环境是先执行 Restrictingfee 数量限制，再执行 FIN_DISCOUNT_FEE 比例折价，最后执行 TOPPRICE 封顶；新规则中心必须以此为准
- **公式不自动跳过限制：** 公式项目是否执行数量、时间窗、同组、同手术限制，以规则动作配置为准；禁止代码层一见公式就跳过全部数量类限制
- **换算数量固定为1：** `换算数量`是固定值；无数量限制时公式使用换算后的 FinalQty，有数量限制时公式使用限制后的 FinalQty
...
- **权威单价：** 当前规则中心只读取 HIS 物价主数据做诊断日志，发现缺价或单价不一致仅记录 Warning，不返回 `PRICE_MISMATCH`，不阻断 simulate/confirm 流程；如未来恢复强校验，必须先确认价格形态、患者事实、合同单位和价格版本来源。
...
1. 数据模型设计（规则表 + 追溯表）
2. 资金安全补齐设计（幂等、状态机、窗口锁、单价诊断、回滚边界）
