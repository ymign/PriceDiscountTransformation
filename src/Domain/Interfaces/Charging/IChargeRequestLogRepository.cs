...
public interface IChargeRequestLogRepository
{
    ...
    Task<ChargeRequest?> GetByIdAsync(long requestId);
    ...
    Task<ChargeRequest?> GetByBusinessKeyAsync(
    ...
    /// <returns>匹配的请求日志实体，不存在时返回 null。</returns>
    Task<ChargeRequest?> GetByFingerprintAsync(string fingerprint);
    ...
    Task<long> InsertAsync(ChargeRequest entity);
    ...
    Task UpdateAsync(ChargeRequest entity);
    ...
    Task<(IReadOnlyList<ChargeRequest> Items, int Total)> GetPagedAsync(
    ...
    Task<IReadOnlyList<ChargeRequest>> GetPendingExpiredAsync(DateTime expireBefore);
}
