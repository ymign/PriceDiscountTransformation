...
public sealed class RulePublishAppService
{
    ...
    public async Task PublishAsync(long ruleId, RulePublishRequest request)
    {
        ...
        await EnsureApprovalPassedAsync(ruleId, request.VersionNo, ApprovalActionCodes.Publish);
    ...
    public async Task DisableAsync(long ruleId, RuleDisableRequest request)
    {
        ...
        await EnsureApprovalPassedAsync(ruleId, header.CurrentVersion, ApprovalActionCodes.Disable);
    ...
    public async Task RollbackAsync(long ruleId, RuleRollbackRequest request)
    {
        ...
        await EnsureApprovalPassedAsync(ruleId, header.CurrentVersion, ApprovalActionCodes.Rollback);
    ...
    private void ClearEffectiveCache()
    {
        ...
        _logger.LogDebug(
            "已清除生效规则缓存、特殊项目标识缓存、互斥动作类型缓存和规则运行期缓存，生效规则缓存键 {EffectiveRuleCount} 个，特殊项目缓存键 {SpecialFlagCount} 个",
