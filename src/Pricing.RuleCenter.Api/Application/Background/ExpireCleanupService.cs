...
public sealed class ExpireCleanupAppService : BackgroundService
{
    ...
    private async Task CleanupExpiredAsync()
    {
        ...
        // GetPendingExpiredAsync 在仓储层实现筛选逻辑，只返回 BusinessStatus="CONFIRM_PENDING"
        ...
        foreach (var log in expired)
        {
            ...
            try
            {
                ...
                await limitRepo.EnsureAndLockAsync(new[] { PricingAppService.BuildRequestLockKey(log.RequestId) });
