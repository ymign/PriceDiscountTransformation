...
/// </summary>
/// <remarks>
/// 公式定义不是直接计算价格的地方。它保存“公式编码、执行器编码、参数结构”等元数据，
/// 供规则动作配置页面约束用户输入，并在计价引擎运行时通过执行器编码找到实际动作。
/// </remarks>
public sealed class FormulaDefService
{
    /// <summary>
    /// 公式定义仓储，负责公式元数据的查询、唯一性判断和启停状态更新。
    /// </summary>
    ...
    /// 初始化公式定义服务。
    /// </summary>
    /// <param name="repository">公式定义仓储。</param>
    ...
    /// </summary>
    /// <param name="formulaId">公式定义主键。</param>
    /// <param name="request">更新请求；不允许修改公式编码，避免破坏已有规则引用。</param>
    ...
    /// 将公式定义实体映射为接口返回对象。
    /// </summary>
    /// <param name="entity">数据库中的公式定义实体。</param>
    /// <returns>公式定义响应 DTO。</returns>
