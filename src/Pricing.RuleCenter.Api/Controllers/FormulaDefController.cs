...
public sealed class FormulaDefController : ControllerBase
{
    ...
    /// </returns>
    [HttpGet]
    public async Task<ApiResult<IReadOnlyList<FormulaDefResponse>>> GetAllAsync()
    {
        var items = await _service.GetAllAsync();
        return ApiResult<IReadOnlyList<FormulaDefResponse>>.Ok(items);
    }
    ...
    public async Task<ActionResult<ApiResult<FormulaDefResponse>>> GetByIdAsync(long formulaId)
    {
        var item = await _service.GetByIdAsync(formulaId);
        if (item is null)
        {
            return NotFound(ApiResult.Fail(404, $"公式定义不存在: {formulaId}"));
        }

        return ApiResult<FormulaDefResponse>.Ok(item);
    }
    ...
    public async Task<ApiResult> ToggleAsync(long formulaId)
    {
