...
public sealed class RuleVersionController : ControllerBase
{
    ...
    /// </returns>
    [HttpGet]
    public async Task<ApiResult<IReadOnlyList<RuleVersionResponse>>> GetByRuleIdAsync(
        long ruleId)
    {
        var items = await _service.GetByRuleIdAsync(ruleId);
        return ApiResult<IReadOnlyList<RuleVersionResponse>>.Ok(items);
    }
    ...
    public async Task<ActionResult<ApiResult<RuleVersionResponse>>> GetByIdAsync(long versionId)
    {
        var item = await _service.GetByIdAsync(versionId);
        if (item is null)
        {
            return NotFound(ApiResult.Fail(404, $"规则版本不存在: {versionId}"));
        }

        return ApiResult<RuleVersionResponse>.Ok(item);
    }
    ...
    [HttpPost]
    public async Task<ApiResult<long>> CreateDraftAsync(long ruleId)
    {
