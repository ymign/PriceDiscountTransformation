...
public sealed class RulePublishController : ControllerBase
{
    ...
    public async Task<ApiResult<IReadOnlyList<RulePublishResponse>>> GetPublishHistoryAsync(
        long ruleId)
    {
        var items = await _service.GetPublishHistoryAsync(ruleId);
        return ApiResult<IReadOnlyList<RulePublishResponse>>.Ok(items);
    }
    ...
    /// </returns>
    [HttpGet("change-logs")]
    public async Task<ApiResult<IReadOnlyList<RuleChangeLogResponse>>> GetChangeLogsAsync(
        long ruleId)
    {
    ...
    [HttpPost("publish")]
    public async Task<ApiResult> PublishAsync(
    ...
    /// <returns>统一成功响应。</returns>
    [HttpPost("rollback")]
    public async Task<ApiResult> RollbackAsync(
