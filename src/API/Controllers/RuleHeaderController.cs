...
public sealed class RuleHeaderController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ApiResult<PagedResponse<RuleHeaderResponse>>> GetPagedAsync(
        [FromQuery] RuleHeaderPagedRequest request)
    {
        var result = await _service.GetPagedAsync(request);
        return ApiResult<PagedResponse<RuleHeaderResponse>>.Ok(result);
    }
    ...
    [HttpGet("{ruleId:long}")]
    public async Task<ActionResult<ApiResult<RuleHeaderResponse>>> GetByIdAsync(long ruleId)
    {
        var item = await _service.GetByIdAsync(ruleId);
        if (item is null)
        {
            return NotFound(ApiResult.Fail(404, $"规则不存在: {ruleId}"));
        }

        return ApiResult<RuleHeaderResponse>.Ok(item);
    }
    ...
    public async Task<ApiResult<IReadOnlyList<RuleHeaderResponse>>> GetByItemCodeAsync(
        string itemCode)
    {
        var items = await _service.GetByItemCodeAsync(itemCode);
        return ApiResult<IReadOnlyList<RuleHeaderResponse>>.Ok(items);
    }
    ...
    [HttpGet("effective")]
    public async Task<ApiResult<IReadOnlyList<RuleHeaderResponse>>> GetEffectiveRules(
        ...
        [FromQuery] DateTime? chargeTime = null)
    {
        ...
        return ApiResult<IReadOnlyList<RuleHeaderResponse>>.Ok(result);
    }
