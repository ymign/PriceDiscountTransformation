...
public sealed class DictController : ControllerBase
{
    ...
    /// </returns>
    [HttpGet]
    public async Task<ApiResult<IReadOnlyList<DictResponse>>> GetByTypeAsync(
        [FromQuery] string dictType)
    {
        var items = await _service.GetByTypeAsync(dictType);
        return ApiResult<IReadOnlyList<DictResponse>>.Ok(items);
    }
    ...
    [HttpGet("{dictId:long}")]
    public async Task<ActionResult<ApiResult<DictResponse>>> GetByIdAsync(long dictId)
    {
        var item = await _service.GetByIdAsync(dictId);
        if (item is null)
        {
            return NotFound(ApiResult.Fail(404, $"字典项不存在: {dictId}"));
        }

        return ApiResult<DictResponse>.Ok(item);
    }
    ...
    [HttpGet("types")]
    public async Task<ApiResult<IReadOnlyList<string>>> GetAllTypesAsync()
    {
        var types = await _service.GetAllTypesAsync();
        return ApiResult<IReadOnlyList<string>>.Ok(types);
    }
    ...
    [HttpPost]
    public async Task<ApiResult<long>> CreateAsync([FromBody] DictCreateRequest request)
    {
    ...
    public async Task<ApiResult> DeleteAsync(long dictId)
    {
