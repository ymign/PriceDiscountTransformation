...
public interface IRuleConditionRepository
{
    ...
    /// <returns>以 (RuleId, VersionNo) 为键的条件集合字典。</returns>
    async Task<IReadOnlyDictionary<(long RuleId, int VersionNo), IReadOnlyList<RuleCondition>>> GetByRuleVersionsAsync(
