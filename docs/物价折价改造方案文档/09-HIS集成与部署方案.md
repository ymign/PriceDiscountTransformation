...
这个菜单是特殊计价规则的唯一编辑入口。原“非药品维护-物价详细信息维护”继续维护项目编码、名称、普通单价、普通单位等基础资料，不再承载折价、公式、双单位、上下限等规则编辑。

原物价维护界面如需联动，只建议提供只读提示：
...
│ 特殊计价规则维护                                  │
├─────────────────────────────────────────────────┤
│ 查询：项目编码 [____] 项目名称 [____] 状态 [▼] [查询] │
├──────────────────────┬──────────────────────────┤
│ 项目列表              │ 项目基础信息（只读）        │
│ CT001 CT平扫          │ 项目编码: CT001            │
│ SKIN001 皮肤治疗      │ 项目名称: CT平扫            │
│ ...                  │ 普通单价: 300.00           │
│                      │ 普通单位: 次               │
├──────────────────────┴──────────────────────────┤
│ 规则列表：[新增] [复制] [编辑] [停用] [发布] [回滚] │
│ [DataGridView: 规则名 | 类别 | 优先级 | 状态 | 版本 | 生效时间] │
├─────────────────────────────────────────────────┤
│ [基础设置] [适用条件] [公式/换算] [限制规则] [部位规则] [互斥/分组] [子项加收] [字典/基础参数] [试算验证] [版本记录] │
│                                                  │
│ 当前 Tab 内容按所选规则展示和编辑                  │
└─────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────┐
│ 就诊类型: [门诊/住院/急诊/全部 ▼]                  │
│ 收费场景: [普通收费/手术批费/补缴费/自助机/微信 ▼] │
│ 年龄条件: [____]  儿童标识: [是/否/不限 ▼]         │
│ 手术号来源: [operationNo ▼]  孕次号来源: [pregnancyNo ▼] │
│ 扩展条件: [DataGridView: 参数名 | 操作符 | 参数值]  │
└─────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────┐
│ [DataGridView: 部位 | 计价类型 | 计价单位 | 计价数量 | 换算单位 | 公式类型 | 优先级] │
│ [新增部位规则] [编辑] [删除] [上移] [下移]          │
└─────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────┐
│ 分组类型: [同组互斥/同手术封顶/同胎次封顶/全院累计 ▼] │
│ 分组编码: [____]    分组名称: [________________]   │
│ 限制口径: [患者+项目/患者+组/手术号+组/孕次号+组 ▼] │
│ 上限数量: [____]    上限金额: [____]               │
│ 组内项目: [DataGridView: 项目编码 | 项目名称 | 是否启用] │
│ [添加项目] [移除项目]                              │
└─────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────┐
│ 子项目编码: [____]  子项目名称: [只读带出]          │
│ 加收方式: [固定金额/比例加收 ▼]                    │
│ 加收比例: [____]%   固定金额: [____]               │
│ 计算基准: [主项目最终金额/主项目原始金额/公式金额 ▼] │
│ 是否自动带出子项目: [是/否 ▼]                      │
└─────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────┐
│ 字典类型: [计价类型/计价单位/换算单位/公式类型/计价部位/规则类别 ▼] │
│ [DataGridView: 编码 | 名称 | 排序 | 是否启用 | 备注] │
│ [新增] [编辑] [停用] [上移] [下移]                  │
└─────────────────────────────────────────────────┘
```

说明：计价相关字典内置在同一个工作台中维护，避免业务人员维护规则时再跳到其他菜单。字典变更需要独立权限控制，停用前必须校验是否已有规则引用。

**试算验证：**

```
┌─────────────────────────────────────────────────┐
│ 患者ID: [____]  项目: CT001  部位: [下拉框▼]       │
│ 数量: [____]    面积: [____]  长度: [____]         │
│ [试算]                                             │
│                                                  │
│ 原始金额: [____]  计算金额: [____]  最终金额: [____] │
│ 命中规则: [____]  折价原因: [________________]      │
│ 计算步骤: [DataGridView]                           │
└─────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────┐
│ [DataGridView: 版本 | 状态 | 发布人 | 发布时间 | 说明] │
│ [查看快照] [回滚到此版本]                          │
└─────────────────────────────────────────────────┘
```
...
| 规则类别 | RULE_CATEGORY | PR_DICT |
| 规则状态 | RULE_STATUS | PR_DICT |
...
3. 保存为草稿，不直接影响收费。
4. 点击试算验证，确认规则结果符合预期。
5. 提交审核或直接发布（按医院权限流程决定）。
6. 发布后生成新规则版本，并主动失效计价服务规则缓存。
7. 收费入口通过统一计价服务使用新版本规则。
```
...
| RULE_VIEW | 查看规则 |
| RULE_EDIT | 新增、编辑草稿 |
| RULE_DICT_EDIT | 维护计价类型、计价单位、公式类型、部位等基础字典 |
| RULE_TEST | 执行试算验证 |
| RULE_PUBLISH | 发布、停用、回滚 |
| RULE_AUDIT_VIEW | 查看变更记录和版本快照 |
...
| 计价服务部署完成 | ☐ | 含健康检查接口 |
| HIS 前端改造完成 | ☐ | 特殊计价规则维护工作台、收费弹窗、结果回填 |
