...
- 存在：由 PricingAgent 接管特殊计价，旧折价逻辑跳过；如果配置错误或计价服务不可用，收费会被阻断，不回退普通计价。
...
- `simulate` 只在门诊收费界面展示阶段使用，不占用额度、不写待提交状态、不新增 HIS 真实子项。
...
最终落账仍以 `Fee.cs` 保存前 `confirm` 返回结果为准。界面预览不会生成 HIS 真实替换/加收子项，避免用户多次修改数量时产生重复子项；真实子项只在 confirm 成功后、HIS 写库前由 `PricingLegacyChargeBridge` 追加。
